...
def main() -> None:
    ...
    # Only show a state that actually has members — a table of zeroes reads as
    # a backlog that still has them.
    zero_rows = "".join(
        f"| {name} | **{c[key]}** | {blurb} |\n"
        for key, name, blurb in [
            ("closed_by_commit_only", "closed_by_commit_only",
             "named in a commit and nowhere else. The weakest state here — "
             "read these before quoting them as done."),
            ("unverified", "unverified",
             "a probe was inconclusive. Treat as unknown, **not** as open."),
        ] if c.get(key))

    unverified_section = (
        "## Unverified — a probe was inconclusive\n\nThese need a code read "
        "before they can be scheduled. Do NOT treat them as open.\n\n"
        + table("unverified")) if c.get("unverified") else ""
    weakest_section = (
        "## Closed by a commit, with nothing in the code naming them\n\nThe "
        "weakest evidence in this file — a promotion to `closed` should start "
