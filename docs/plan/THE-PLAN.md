...
| 2.5 ◐ | **Give the orphan screens a home (D2).** ⚠️ **Half of this landed with 2.4**: naming a screen IS a way in, so `every-screen-has-a-way-in.test.ts` now counts the ⌘K inventory and the three `compliance/*` exemptions it carried are deleted. That guard also had to learn to read `screens.ts` as DATA rather than scan it for links — its `UNLISTED` keys are path literals, and a file explaining why a screen is unreachable must not be what makes it look reachable, which is the stripComments problem in another form. **What remains is PLACEMENT**: a screen reachable only by typing its name is reachable, not discoverable | 1d | every route sits under a hub tile or a menu, not just in the palette |
