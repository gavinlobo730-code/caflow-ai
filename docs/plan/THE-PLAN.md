...
| D27 | Emailing a client's customers | **Never automatic.** A Send button, always |
...
| **D27** | May the nightly sweep EMAIL a client's own customers? (SALES-23) | **No — the CA presses send** | Owner, 25-09, confirming the un-numbered row above and closing SALES-23. It sends mail outward on the CLIENT's behalf, to people who are strangers to the engagement, over a figure the sweep computed and nobody read. So the bulk Remind on the sales screen stays the only door, and `a-bulk-email-is-bounded-and-nothing-schedules-it.test.ts` keeps it that way. **Nothing in the code changes**: the sweep already emails nothing and the guard already says so — what was missing was the record, which is why STUCK.md §2 still listed it as open |
| **D28** | What does a client screen render when the id resolves to nothing? | **A named not-found state, per screen** | Owner, 25-09, from STUCK.md §3's two options: *"Each section says 'Sales — no such client' instead of spinning. Better for a real CA hitting a dead bookmark, and the gate closes as a side effect rather than by exemption."* Chosen OVER exempting the placeholder group from the smoke walk's duplicate-body check, which would have removed six screens from that guard's reach for good. Built as ONE gate in `app/clients/[id]/layout.tsx`, which all forty routes share, over one resolution hoisted into `ClientNavProvider`. `MAX_ROUTES_PER_DIGEST` is unchanged at 5 |
| **D29** | Cost centres as a dimension on `journal_lines`? (ACC-13) | **Build it** | Owner, 25-09. A real dimension a practice asks for. Migration 418: a `cost_centres` master and a NULLABLE `journal_lines.cost_centre_id`, with `post_journal_atomic` and `edit_posted_journal` derived from **migration 384**, the last definer of both. Nullable, no backfill, for migration 411's reason — every line already posted was written by a door that did not know the column, so a default would assert a centre nobody chose |
| **D30** | Which figures does `client_period_metrics` hold? | **Twelve columns** | Owner, 25-09: turnover, profit before tax, tax expense, output tax, ITC availed, ITC reversed, GST cash paid, purchases, TDS deducted, TDS deposited, payroll cost, employee count. A benchmark is only as good as the history the FIRST migration starts capturing, so the column list is chosen once — widening it later loses the years in between |
...
| 2 | Backend and frontend suites green, smoke walk renders every route | **PASSING as of 25 Sep (D28).** The walk now reports *166 screens walked, 0 with a problem · **160 distinct bodies across the 160 routes that stayed put*** — no duplicate group at all, where it used to fail on six. `MAX_ROUTES_PER_DIGEST` was NOT raised and the placeholder group was NOT exempted: a client route whose id resolves to nothing renders a named refusal naming the screen and the address, so the six identical bodies became forty different ones. ⚠️ The first draft named only the MODULE and turned six identical bodies into five smaller groups of identical ones — passing the check with zero headroom, which is the exemption this was chosen over wearing a different hat. Suites: 17,494 backend · 1,566 real-Postgres · frontend lint/typecheck/test/build green |
