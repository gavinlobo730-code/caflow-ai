...
| Named-colour utilities | **5,541** | 5,840 earlier on 24 Sep; 5,993 before that | `for c in amber red green emerald blue gray slate rose yellow orange teal; do grep -rEoh "\b(text\|bg\|border\|ring\|fill\|stroke\|divide\|from\|to\|via)-$c-[0-9]{2,3}\b" apps/web/{app,components,lib}; done \| wc -l` |
