...
| Named-colour utilities | **4,376** | 5,541 and 5,840 earlier on 24 Sep; 5,993 before that | `for c in amber red green emerald blue gray slate rose yellow orange teal; do grep -rEoh "\b(text\|bg\|border\|ring\|fill\|stroke\|divide\|from\|to\|via)-$c-[0-9]{2,3}\b" apps/web/{app,components,lib}; done \| wc -l` |
...
| **2.2** ◐ | **The hub — firm level and client level.** The 15 tiles of D1, each with live signal rather than a label. **The AUTHORITY landed 24-09** (`domain/hub/tiles.py` + 36 tests): what each tile ASKS, in what unit, and which tiles honestly have none. **The signal is what needs DOING, not what exists** — a tile reading "1,284 invoices" is unactionable, so every figure is outstanding work, which also fixes the direction (**lower is better on all fifteen**, so a CA never has to remember which way one reads) and a guard rejects a question that reads as a total. **Two tiles have no signal and say so**: `insights` and `reports` are destinations, not queues, and `describe()` FORCES their figure to None so a service cannot put a number on one by mistake — a nil meaning *nothing to do* and a nil meaning *nobody can tell* are different facts, and the payload keeps three states apart (0 / null-and-flagged / null-and-not, the third being a failed fetch), asserted as three distinct dicts. ⛔ **The href guard found the thing that changes the design**: it opens each destination against the real route tree rather than trusting the path, four were wrong, and chasing them found **five modules with no firm-level screen at all** — Banking, Purchases, Inventory, Year-End, and **Fixed Assets, whose `/accounting/fixed-assets` is a `MovedToClientWorkspace` TOMBSTONE**, which is worse than a 404 because it renders. That is **question G3**, with the recommendation (build the five) and the fact that makes it cheap: a firm-level roll-up is a STATIC route, so it costs **none** of D10's dynamic rules. **Still to build: the service, the endpoint, the two screens.** | 3–4d | Every tile shows a real figure or a real count; none is a stub. Four negative controls on the authority, each firing on one test |
| **2.2b** | **The hub SCREEN, firm level and client level — what is left of 2.2.** The authority and the service landed 24-09; this is the half a CA sees, and the endpoint lands WITH it because `test_every_mounted_endpoint_has_a_way_in` refuses a mounted route no screen calls (budget 0, and raising it would ship the shape the ratchet exists to prevent). **What `/` becomes is the decision inside this item.** Today it is `DashboardContent`, 594 lines of greeting + upcoming deadlines + recent clients + pending tasks, reading four tables straight over PostgREST. That is not a module hub — it is one module's worth of signal on the front page, and three of its four reads are the unpaged browser pattern D11's sweep left behind. **The hub replaces it**, and the three panels it carries are not lost: Deadlines is the `compliance` tile's own destination, and Clients and Tasks are two more tiles' worth of the same idea. **One request, not fifteen** — `GET /api/hub` is built and returns every tile with its figure, so the screen fetches once. **Three renderings, not one**: a tile with a figure, a tile whose figure is a named absence (`answerable: false`, render the sentence — `insights`, `reports`, and `inventory` at firm scope), and a tile whose fetch failed (`answerable: true`, `signal: null` — say so rather than showing a zero). **A tile with no `href` at this scope is not a link** (G3's five at firm level) and must not look like one. ⚠️ **The smoke walk is the acceptance test**, not a screenshot: `/` is on 90+ of the 160 routes' path and the owner's standing concern is that screens should not look odd — so a 160-route walk before and after, pixel-diffed, the way G0 and D11 were verified | 2–3d | The hub renders at both scopes from one request; no tile is a stub; a 160-route smoke walk before/after with every moved screen read |
