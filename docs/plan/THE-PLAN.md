...
| 2.7c | **The remaining per-module conversion** | 3–6d | Smoke walk green per module. Budget 0.5–1d per module for guard edits — several guards name a file path and will break on a move that does not break their rule; restate them as the rule. ⚠️ Four such restatements were needed in 2.7a–b alone, three of them the SAME hazard: a guard that reads prose cannot tell the explanation from the deed |
