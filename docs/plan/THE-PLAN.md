...
|---|---|---|---|
| **1.1** | **The type scale (D12).** Convert all arbitrary sizes, MODULE BY MODULE, each with a smoke-walk diff. **1.1a landed 24-09: the auth and onboarding family**, 103 of 380 in six files — its own 32/26/22/18/16/14/13/12 ladder, three steps of it sitting between Tailwind's. Remaining: the SHELL (13 × `text-[12.5px]`, a half pixel, in `ActivityRail` and the seven `ContextPanel` panels), payroll (~80), the dashboard, and 30 × `text-[9px]` which T3-a decided gets no name | 2–3d | `grep -rEoh 'text-\[[0-9]+(\.[0-9]+)?px\]' apps/web/app apps/web/components apps/web/lib \| wc -l` = **0** (the DECIMAL and `lib/` matter — the old spelling of this metric missed 13 half-pixel sizes), and the guard's budget is 0. **289 today** |
