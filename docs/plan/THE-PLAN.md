...
| **1.3** | **The named-colour judgement pass (T4-b).** ~5,350 utilities like `text-amber-600`. Status colours resolve to the semantic tokens; decorative ones stay. **1.3a landed 24-09 — the slice where the judgement is not a judgement**: an element carrying `role="alert"` has ANSWERED the status question out loud, so 59 raw reds across 29 files resolve, and the money-direction tokens leave the five status sites they had been reached for as "a red and a green". Two of those reds were **below WCAG 1.4.3** on the surface they sat on. The rest of T4-b is still a judgement per site | 2–3d | `amber`/`red`/`green` on a STATUS element resolve to `state.*`, asserted by a guard. Three rules are asserted today: no money token on an alert, no money token outside the module where money has a direction, no raw status colour inside an alert |
