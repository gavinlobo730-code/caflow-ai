...
| ID | item | DONE WHEN |
|---|---|---|
| 3a-1 ✅ | The **Insights** tile (D1) — health, risk, profitability, trends in one place | **DONE 25-09.** The tile's `firm_href` was `/health`, which is ONE of the three things it names — a client health monitor — so two thirds of its own question had nowhere to land. `/insights` now holds the recommendations, the compliance risk and the relationship health; profitability is a question about the PRACTICE rather than a client and sits at `/practice/profitability` (3a-2), which the page LINKS to rather than duplicating. It belongs to the **Home** workspace: the tile asks the morning question across every client, which is what Home is for. Trends are 3c and are not claimed. | 
...
| 3a-4 ✅ | Health — render the **computed** 7-dimension score | **DONE 25-09, AND THE ROW WAS ASKING FOR THE WRONG THING.** Re-read as it said to: the screens do NOT rebuild a 7-dimension score in the browser, and of the four unreached `/api/health` endpoints three (`dashboard`, `overrides` GET, `scores/{id}/history`) are the recorded read-over-PostgREST convention. What the re-read found instead is worse than an unreachable endpoint. **`POST /scores/{id}/override` has existed since migration 059, both screens call it, both screens LIST the rows under "Active Overrides" — and `_calculate_scores_db` never read the table**, so the CA corrected a dimension, saw the correction listed, and the number they were correcting did not move. Three more fell out of the same reading: `DELETE /overrides/{id}` was written, guarded and had NO caller, so an override with no end date could never be withdrawn; `expires_at` was checked by nothing, so one that lapsed in March still read as live in September; and **the two screens spoke different dimension vocabularies** — the client tab's picker offered the legacy flat columns (`relationship_risk_score`, …), which are not the seven the engine matches on, so every override recorded there named something that does not exist. `domain/health/overrides.py` is the rule and `domain/health/scoring.py` is Chapter 16's weights MOVED out of the router (a domain module may not import one — `schedule_ii.py`'s reasoning), re-exported so no caller changed. **The manual override is applied FIRST and the hard override AFTER**, so a CA can raise a score and still cannot mask a Critical condition — which `create_override`'s own comment names as the thing to prevent. ⚠️ **One trap the change had to dodge**: the computed dict is spread into the `health_scores` upsert at two sites and into two history snapshots, and `health_scores` has no column for the override working — so `_columns_only` strips it at all four, the `form_26as_reconciliations` shape migration 291 had to repair. Measured first: production holds ZERO `health_overrides` rows, so this was latent and fires the first time the control is used. Budget 4 → 2, total 228 → 226. |
| 3a-5 ✅ | Compliance risk and predicted misses | **DONE 25-09.** All six `/api/intelligence/*` endpoints had `lib/api` methods and ZERO screen callers; `/insights` reaches the three client-facing reads and the budget drops 6 → 3 (`workload-insights` and the two journal-suggestion doors are still owed a screen). ⚠️ **THE ONE THING CHECKED BEFORE SURFACING ANY OF IT**: this file records that "the modules named intelligence and memory analyse TASKS, not money" and that `cash_flow_risk_months` is literally the two months with the most tasks — so surfacing a financial-sounding figure derived from task volume would have been worse than leaving the endpoints unreachable. It was measured, not assumed: `cash_flow_risk_months` and `seasonal_revenue_peak` live in `memory_repository`, NOT in `intelligence_service`, so 3b-3's concern is not surfaced here (and both were retired the same day — see 3b-3); `risk_score` is built from the client's own compliance records (overdue count, due inside seven days, history of filing late); and `outstanding_paise` is summed off invoices with status Issued or Overdue. Real records and real money. | 
| 3a-6 ✅ | `ai_insights` — wire the **writer** | **DONE 25-09, and the premise was exactly right.** `domain/ai_insight_service.generate_insights_for_client` is written and tested, `POST /api/ai-insights/generate/{client_id}` mounts it, and `api.aiInsights.generate` carried the method in `lib/api` with NO caller — so the client AI Insights screen read `ai_insights` over PostgREST and showed an empty list for every client, for ever, with a working writer one button away. ⚠️ **AND THE EMPTY STATE SAID SOMETHING FALSE**: *"Insights are generated automatically as activity is recorded for this client"* — nothing generates them, no job, no trigger, no posting path, so it told a CA to wait for something that was never going to happen. A Generate button gated on `can("report","write")` with `resolved`, the endpoint's own permission; the sentence now says who can generate them and that nothing does it on its own. | 
| 3a-7 ✅ | Give the copilot the client's own data | **DONE 25-09, and the ORDER was the point.** `routers/assistant` was a pure Groq passthrough over a static SYSTEM_PROMPT that loaded no client data. ⚠️ **ITS OWN CODE HAD WRITTEN DOWN THE CONDITION FOR CHANGING THAT**: a `client_id` field once sat on the request model read by nothing and was DELETED with the reason in its place — *"this router carries NO mount-level client guard ... so a dead client_id is a trap: the moment someone wires it up to real client context, there is nothing enforcing assignment scope. If the assistant ever becomes client-aware, add `_CLIENT_GUARD` to its `include_router` FIRST."* That was done first. **Two independent checks**, deliberately: the mount guard (`require_client_access`, which inspects a JSON POST body as well as path and query) and the handler's own `assert_client_access` — the mount guard is one line in another file and a refactor that never opens the router can drop it, and this is the one endpoint in the product that forwards a client's figures to a third party. Each has its own negative control. **`domain/ai/client_brief` is the rule**, built from the HUB payload so a tile added later reaches the copilot the day it is added and there is no second list to drift; Phase 3's rule travels *inside* the block rather than only in the system prompt, so a later prompt edit cannot separate the figures from the instruction not to compute on them. **Four things are refused and named**: no document text (a statement line names a counterparty who is a stranger to the engagement), no employee record, no GSTIN/PAN/TAN, and no figure from another client — the last structurally, since the builder takes one payload produced for one validated id, asserted on the SIGNATURE so a fourth argument fails rather than becoming a way in. The screen gets a per-conversation client picker that is **not persisted** (a CA returning tomorrow must not silently send yesterday's client's figures) and a line saying what leaves the building. ⚠️ **`test_router_client_scope` caught the exemption going stale the moment the handler first mentioned `client_id`** — `/api/assistant` was listed as "has no client-scoped resource", every clause of which had become false. That is the guard that stopped an exemption outliving its reason. | 
...
| 3b-2 ✅ | **Real cash-flow forecasting** from invoice and bill due dates, ageing and bank history | **DONE 25-09.** `domain/cash_flow/forecast.py` is the rule, `services/cash_flow_service.py` fetches, `routers/cash_flow_forecast.py` decides nothing and `/reports/cash-flow` renders. ⚠️ **THE ROW UNDERSTATED IT: THE BROWSER FORECAST'S ONLY INFLOW WAS `fee_invoices` — the PRACTICE'S OWN FEE NOTES to that client** (migration 014: `engagement_id` to `fee_engagements`, GST on SAC 998211). So a client turning over crores was shown a cash-flow forecast whose entire income was the fee they pay their accountant; `client_sales_invoices` never appeared, and neither did a single supplier payable. The typed opening balance the row DID name is the second half: every closing figure is carried forward from it, and it now comes off the client's own bank and cash ledgers through `period_net_by_account` — one bucket read, so a client with 12,836 entries costs what one with ten costs. **Three judgements, each tested**: an OVERDUE document is reported apart and reaches no month (dropping it into the first month flatters the forecast exactly where it is relied on); a document with NO DUE DATE is named rather than derived from the party's credit days; and a carried-over `is_opening` document COUNTS, which is the opposite of what `without_carried_over` does for every statutory output and is said so explicitly. **The statutory and payroll legs are NAMED on every answer and not priced** — a return for a month that has not happened cannot be priced, and pricing only the prepared months would show a dip where the CA happened to have worked ahead. **This month's loan EMI is reported, not counted**: nothing records the EMI day and the opening position already reflects anything paid, so both readings are given rather than one chosen. A `draft` invoice is excluded BY STATUS and would not fall out on its own, because `outstanding_paise` is generated from total less paid. Own router, not `/api/accounting/reports/*`: that prefix serves the AS-3 cash flow STATEMENT (migration 277), and two things called cash flow under one prefix is how one gets quoted at a meeting about the other. |
| 3b-3 ✅ | Rewrite or retire `cash_flow_risk_months` and `seasonal_revenue_peak` | **RETIRED 25-09.** Both, plus a third the row did not name: `avg_year_end_duration_days` was the literal `30` commented "default estimate" on a column whose name says it was measured. **The visible artefact was not the field** — `detect_cash_flow_warnings` read `cash_flow_risk_months` back and raised a card headed "Cash Flow Pressure Period: &lt;month&gt;" at 72% confidence asserting "elevated tax obligations and operational costs", off a task count; the detector is deleted and the `/detect` bundle no longer serves a permanently empty key. **MEASURED, NOT ASSUMED**: production holds 232 `client_profiles` rows (6 current, 5 with `data_points_used` > 0, so the pipeline runs) and NOT ONE carries either field, and `ai_memory_triggers` has never held a `cash_flow_warning` — because `public.tasks` is empty. **Latent, and it fires on the first client task anybody creates**, which is the demo path. ⚠️ **RETIRE, not rewrite, and the reason is 3b-2**: revenue by month IS cheap (`account_period_balances` through `period_net_by_account`, N windows off one bucket read) but seasonality is an INPUT to a cash-flow forecast, not a memory-profile field — half of it hung off `client_profiles` would be in the wrong place and would then have to move. A cash-flow RISK month cannot be derived at all today. **Nothing was lost, a wrong label was**: `compute_firm_profile` does the identical arithmetic for `peak_workload_months` and `capacity_risk_months`, names that are true of a task count, and a test pins that it survives. Two defects found in that surviving copy: it took the **UTC** month of an aware stamp (20:00 UTC on 31 March is 1 April in India — a whole month out at a boundary), and `common_auditor_requests` was `list({...})[:5]`, so the stored list changed between nightly sweeps under hash randomisation. `deadline_concentration` is deliberately NOT converted and says so — `due_date` is a DATE. |
