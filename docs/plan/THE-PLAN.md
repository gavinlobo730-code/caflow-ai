...
| 3a-4 ⚠️ | Health — render the **computed** 7-dimension score | ⚠️ **RE-MEASURED 25-09 AND THIS ROW IS STALE — read before scheduling it.** It says "8 unreached health endpoints"; the reachability ratchet says **4**. And the screens do NOT rebuild a 7-dimension score in the browser — `/health` filters on a stored `overall_score`, and `recalculate-all`, `dimension-detail`, `scores/{id}/override` and `alerts/{id}/resolve` are already called, with in-code comments recording the deliberate choice to read the DATA over PostgREST and only the ACTIONS through the API. The item is smaller than written; re-read it against the code before booking the time. | 
