...
import { Button, Section, SectionHeading, Card, IconBadge } from "@/components/ui";
  ...
  return (
    <>
      ...
      <div className="border-y border-ps-border bg-white">
        <div className="container-ps flex flex-col items-center gap-6 py-9 md:flex-row md:justify-between">
          <Reveal variant="left">
            <p className="text-[14px] font-semibold text-slate-500">One platform instead of five</p>
          </Reveal>
          <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-2">
            {REPLACES.map((name, i) => (
              <Reveal key={name} variant="scale" delay={i * 90}>
                <span className="rounded-full border border-ps-border bg-ps-bg px-3.5 py-1.5 text-[13px] font-medium text-slate-400 line-through decoration-rose-300/80">
                  {name}
                </span>
              </Reveal>
            ))}
            <Reveal variant="scale" delay={REPLACES.length * 90}>
              <span className="flex items-center gap-3">
                <ArrowRight size={18} className="text-slate-300" />
                <span className="btn-shine rounded-full bg-brand px-4 py-1.5 text-[13px] font-semibold text-white">
                  PracticeSync
                </span>
              </span>
            </Reveal>
          ...
          {WHY.map((item, i) => (
            <Reveal key={item.title} delay={i * 110}>
              <Card className="card-lift h-full">
                <span className="icon-pop inline-grid h-11 w-11 place-items-center rounded-xl bg-brand/[0.06] text-brand ring-1 ring-brand/10">
                  {item.icon}
                </span>
                <h3 className="mt-5 text-[17px] font-bold text-brand-dark">{item.title}</h3>
      ...
      <Section id="products" className="bg-white">
        ...
        <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {MODULES.map((m, i) => (
            <Reveal key={m.title} delay={(i % 3) * 110 + Math.floor(i / 3) * 60}>
              <Card className="card-lift h-full">
                <span className="icon-pop inline-grid h-11 w-11 place-items-center rounded-xl bg-brand/[0.06] text-brand ring-1 ring-brand/10">
                  {m.icon}
                </span>
      ...
      <Section className="bg-white">
        <div className="grid gap-8 rounded-3xl border border-ps-border bg-ps-bg p-10 sm:grid-cols-2 lg:grid-cols-4">
          {[
