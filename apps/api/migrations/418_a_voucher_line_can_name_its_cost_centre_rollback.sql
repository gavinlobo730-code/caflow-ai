-- Rollback for 418. Restores each function to the body its LAST DEFINER before
-- 418 held — post_journal_atomic and edit_posted_journal from migration 384,
-- assert_journal_lines_belong_to_the_client from migration 360 — and drops the
-- column and the master.
--
-- Each is that migration's text VERBATIM, taken by the same script that
-- derived 418's versions from it, so a rollback cannot silently restore
-- something neither migration ever contained. `tests/test_a_voucher_line_can_
-- name_its_cost_centre.py` asserts each one equals its ancestor exactly and
-- that not one mentions a cost centre.
--
-- ⚠️ THE ORDER MATTERS. The trigger must stop reading `cost_centres` BEFORE the
-- table is dropped, and `journal_lines.cost_centre_id` must go before the
-- master it references. A rollback that dropped the table first would leave a
-- trigger referring to a relation that no longer exists, and every posting
-- would fail.

BEGIN;

-- ── post_journal_atomic ── migration 384's body, verbatim ───────────────────
...
-- ── edit_posted_journal ── migration 384's body, verbatim ───────────────────
...
-- ── the tenancy trigger ── migration 360's body, verbatim ───────────────────
...
ALTER TABLE public.journal_lines DROP COLUMN IF EXISTS cost_centre_id;
