...
#
# FA-20 moved them. `services/reconciliation_service.py` runs the same
# register-integrity checks in the nightly sweep now, and a service reaching
# into a router for a statutory table is the wrong direction and one refactor
# from an import cycle. What stays here is the re-export, because this module
# is where the endpoints are and where four tests already import them from.
SCHEDULE_II_RESIDUAL_FRACTION = schedule_ii.SCHEDULE_II_RESIDUAL_FRACTION
_SCHEDULE_II_PART_C = schedule_ii.PART_C
_NOT_DEPRECIABLE = schedule_ii.NOT_DEPRECIABLE
_wdv_rate_for_life = schedule_ii.wdv_rate_for_life
SCHEDULE_II_CATEGORIES = schedule_ii.CATEGORIES
_default_schedule_ii_class = schedule_ii.default_class
_DEFAULT_WDV_RATES = schedule_ii.DEFAULT_WDV_RATES
schedule_ii_departure = fa_integrity.schedule_ii_departure
...
_INTEGRITY_COLUMNS = fa_integrity.COLUMNS


def _register_rows(db, firm_id: str, client_id: str) -> list[dict]:
    """Every live asset in one client's register, paged."""
    return fetch_all(
        lambda: (db.table("fixed_assets").select(_INTEGRITY_COLUMNS)
                 .eq("firm_id", firm_id).eq("client_id", client_id)
                 .is_("deleted_at", "null")),
        label="fixed_assets.register_integrity")


def _live_bill_ids(db, firm_id: str, rows) -> set:
    """Which of the purchase bills these assets were capitalised from still exist.

    Resolved here rather than in the domain rule because only this side has a
    database. Batched at 200 because `in_` builds a URL and PostgREST has a
    length limit on one.
    """
...
def register_integrity(
    ...
    rows = _register_rows(db, firm_id, client_id)
    findings = fa_integrity.register_findings(
        rows, live_bill_ids=_live_bill_ids(db, firm_id, rows))
