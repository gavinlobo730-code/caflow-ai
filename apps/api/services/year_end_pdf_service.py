...
def group_indian_rupees(amount_rupees: int) -> str:
    """Integer RUPEES (not paise) grouped the Indian way, sign kept."""
    sign = "-" if amount_rupees < 0 else ""
    return f"{sign}{group_indian(str(abs(amount_rupees)))}"


def _rs(paise: int) -> str:
    """Whole rupees, grouped the Indian way, for the statements.

    `_paise_to_rupees` and `_format_indian` MOVED to
    `domain/reporting/pdf_money` when the invoice and the payslip turned out to
    need the same grouping and to have got it wrong. This module's version was
    the one that was right — it took `abs()` before splitting — which is why it
    is the one that moved.
    """
    return whole_rupees(paise)


# The two names the rest of this module (and `_presentation`) import.
_format_indian = group_indian_rupees
...
def _cover_page(elements, st, eng: dict, doc_title: str, is_draft: bool):
    ...
    # IST, NOT THE SERVER'S CLOCK. `apps/api` runs on Render in SINGAPORE
    # with a UTC clock, so a naive `datetime.now()` dated a pack built at
    # 01:00 IST on 1 April as 31 March — the previous DAY and, on that one
    # night of the year, the previous FINANCIAL YEAR, on the cover of a set
    # a CA signs.
