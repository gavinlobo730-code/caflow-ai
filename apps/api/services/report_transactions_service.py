...
def gst_summary(db, firm_id: str, client_id: str, month: str) -> dict[str, Any]:
    """GSTR-1 / GSTR-3B figures for one client and one calendar month.

    month is 'YYYY-MM'. The window is built from the month itself rather than a
    hardcoded 31-day end — the previous implementation used `${month}-31`, which
    silently dropped nothing only because Postgres compares date strings, but
    would have been wrong the moment it was used as a real date.

    CGST Act Section 49 — output tax less eligible input tax credit is the net
    liability discharged from the electronic cash/credit ledger.
    """
    y, m = (int(p) for p in month.split("-", 1))
    end_y, end_m = (y + 1, 1) if m == 12 else (y, m + 1)
    start = f"{y:04d}-{m:02d}-01"
    end_exclusive = f"{end_y:04d}-{end_m:02d}-01"

    rows = list_transactions(db, firm_id, client_ids=[client_id], date_from=start)
    rows = [r for r in rows if (r["transaction_date"] or "") < end_exclusive]

    sales = [r for r in rows if r["transaction_type"] == "sales_invoice"]
    purchases = [r for r in rows if r["transaction_type"] == "purchase_invoice"]

    def total(rs: list[dict], field: str) -> int:
        return sum(int(r.get(field) or 0) for r in rs)

    out_cgst, out_sgst, out_igst = (total(sales, f) for f in
                                    ("cgst_paise", "sgst_paise", "igst_paise"))
    itc_cgst, itc_sgst, itc_igst = (total(purchases, f) for f in
                                    ("cgst_paise", "sgst_paise", "igst_paise"))
    ...
    return {
        ...
        "gstr1": {
            "taxable": total(sales, "taxable_amount_paise"),
            "cgst": out_cgst, "sgst": out_sgst, "igst": out_igst,
            "total": total(sales, "total_paise"),
            "lines": sales,
        },
        ...
        "gstr3b": {
            ...
            # Section 49 set-off order is NOT modelled here: these are the raw
            # per-head differences the screen has always shown. A negative value
            # means credit carried forward, and is left signed rather than
            # floored so the screen cannot silently hide a carry-forward.
            "net_cgst": out_cgst - itc_cgst,
            "net_sgst": out_sgst - itc_sgst,
            "net_igst": out_igst - itc_igst,
        },
        "tds_deducted": total(purchases, "tds_paise"),
        # CA REVIEW REQUIRED — DO NOT AUTO-SUBMIT. This is a working figure for a
        # screen, not a filed return; the filing path is /api/gst/*/from-books,
        # which additionally reconciles to the General Ledger.
