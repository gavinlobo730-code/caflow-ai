    ...
    <div className="bg-white rounded-xl border border-[#F1F5F9] overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-50 flex items-center justify-between gap-3">
        ...
        <select value={fy} onChange={(e) => setFy(e.target.value)}
          className="px-2.5 py-1.5 border border-[#E2E8F0] rounded-lg text-xs bg-white">
          {years.map((y) => <option key={y} value={y}>FY {y}</option>)}
        </select>
      </div>

      {loading && (
        <div className="px-5 py-8 flex justify-center text-[#94A3B8]">
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-[#F1F5F9]">
            {[
              ...
              <div key={label} className="bg-white px-4 py-3">
                ...
                <p className="text-sm font-semibold text-[#0F172A] tabular-nums mt-0.5">{value}</p>
              </div>
            ))}
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-[#F8FAFC]">
                {data.months.map((m) => (
                  <tr key={m.month}>
                    <td className="px-5 py-2 text-[#1E293B]">{MONTH_LABEL(m.month)}</td>
          ...
          {data.gaps.length > 0 && (
            <div className="px-5 py-3 border-t border-[#F1F5F9] space-y-1.5">
              {data.gaps.map((g, i) => (
                <p key={i} className="text-[11px] text-[#64748B] flex gap-1.5">
                  <Info size={12} className="shrink-0 mt-0.5 text-[#94A3B8]" />
                  <span>{g}</span>
                </p>
              ))}
            </div>
