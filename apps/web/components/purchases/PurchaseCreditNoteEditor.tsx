...
/**
 * PurchaseCreditNoteEditor — dedicated create/edit experience for Purchase
 * Credit Notes (CGST Act §34(3), the increase-side mirror of Debit Notes: a
 * vendor undercharged us, we now owe more). Structural mirror of
 * DebitNoteEditor.tsx — same architecture, same field set, minus the "exceeds
 * outstanding" warning (a credit note has no ceiling to preview against,
 * unlike a debit note capped by the bill's outstanding balance).
 *
 * Once issued, a credit note is immutable except notes/document_url (CGST
...
/** Server line shape (from GET /api/purchase-credit-notes/{id}). */
export interface PurchaseCreditNoteLineDetail {
  id?: string;
  description: string;
  hsn_sac: string | null;
  quantity: number;
...
export interface PurchaseCreditNoteDetail {
  id: string;
  vendor_id: string;
  ...
  lines: PurchaseCreditNoteLineDetail[];
}

function detailLinesToEditorLines(lines: PurchaseCreditNoteDetail["lines"]): EditorLine[] {
...
export function PurchaseCreditNoteEditor({
  ...
  existing?: PurchaseCreditNoteDetail | null;
  /** "Duplicate credit note" prefill — create mode only. purchase_bill_id is
   * deliberately NOT copied (see lib/purchases/purchaseCreditNoteDuplicateSeed.ts). */
  duplicateSeed?: PurchaseCreditNoteDetail | null;
  ...
  const currentSnapshot = { vendorId, cnDate, reason, notes, billId, isInterstate, isReverseCharge, lines };
  ...
  const totals = previewPurchaseCreditNoteTotals(lines, isInterstate);
  const validation = validatePurchaseCreditNoteEditor({ vendorId, creditNoteDate: cnDate, lines });
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/purchase-credit-notes/upload`, {
        method: "POST",
  ...
  async function save() {
    ...
    try {
      ...
      if (isEdit && existing) {
        ...
        const upd = await apiCall(`/api/purchase-credit-notes/${existing.id}`, "PATCH", patchPayload, token);
        ...
        <section className="bg-white rounded-xl border border-[#F1F5F9] p-4">
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
            <div>
              ...
              {isEdit && <p className="mt-1 text-[10px] text-[#94A3B8]">Vendor can&apos;t be changed once a credit note exists.</p>}
