        ...
        <div className="p-5 space-y-5">
          ...
          <section className="space-y-2.5">
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              <span className={`px-2 py-0.5 rounded-full text-3xs font-medium ${CN_STATUS_BADGE[cn.status] ?? "bg-ps-muted text-ps-label"}`}>
                {cn.status}
              </span>
            </div>
            ...
            {showJournal && (
              <div className="border border-ps-muted rounded-lg p-2 bg-ps-bg">
                {journalLoading ? (
                  ...
                  <p className="text-2xs text-ps-hint py-1">Journal {cn.journal_entry_id} — line detail unavailable here.</p>
                )}
              </div>
            )}
          </section>
