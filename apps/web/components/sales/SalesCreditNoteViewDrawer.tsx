          ...
          <section className="space-y-2.5">
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="text-right flex-shrink-0">
                <p className="text-base font-semibold text-ps-ink font-mono">{fmt(cn.total_paise ?? 0)}</p>
                <p className="text-[10px] text-ps-hint">Credit note total</p>
              </div>
            </div>

            <div className="flex flex-wrap items-center gap-1.5">
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${CN_STATUS_BADGE[cn.status] ?? "bg-ps-muted text-ps-label"}`}>
                {cn.status}
              </span>
            </div>
            ...
            {showJournal && (
              <div className="border border-ps-muted rounded-lg p-2 bg-ps-bg">
                {journalLoading ? (
                  ...
                  <p className="text-[11px] text-ps-hint py-1">Journal {cn.journal_entry_id} — line detail unavailable here.</p>
                )}
              </div>
            )}
          </section>
