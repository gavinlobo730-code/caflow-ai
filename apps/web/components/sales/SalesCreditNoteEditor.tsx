...
/**
 * SalesCreditNoteEditor — dedicated create/edit experience for Sales Credit
 * Notes, mirroring SalesDebitNoteEditor's architecture (InvoiceWorkspaceLayout,
 * dirty-guard, live GST preview, per-line Unit, EntityLookup invoice picker).
 * No business logic is duplicated: totals shown here are a preview (lib/
 * invoices/gst.ts's previewTotals, matches credit_notes.py's
 * _compute_line_gst exactly — same rule as sales_invoices.py); the backend
 * remains authoritative and recomputes everything on save.
 *
 * Unlike a Sales Debit Note, a credit note DOES have a ceiling: it reduces
 * what the customer owes and cannot exceed the linked invoice's outstanding
 * (CGST Act §34 — a credit note corrects an existing supply's value and
 * cannot exceed it; routers/credit_notes.py's issue_credit_note enforces
 * this server-side with a 422). The summary previews that ceiling the same
 * way DebitNoteEditor.tsx (Purchase Debit Note — the other capped note
 * type) does, since the server remains authoritative either way.
 *
 * Deliberately mirrors the Sales Invoice baseline, not the Purchase Credit
 * Note: no attachment/upload section (the Sales Invoice editor has none
 * either — parity means matching the baseline, not exceeding it).
 *
 * Once issued, a credit note is immutable except `notes` (CGST Act §34 —
 ...
 * (query-side) to non-draft, non-cancelled: a credit note corrects an
 * invoice that was actually issued (CGST Act §34), matching the same
 * status filter Sales Debit Note's "Against Invoice" picker applies. */
...
/** Server line shape (from GET /api/credit-notes/{id}). */
export interface SalesCreditNoteLineDetail {
  id?: string;
  description: string;
  hsn_sac: string | null;
  quantity: number;
...
function detailLinesToEditorLines(lines: SalesCreditNoteDetail["lines"]): EditorLine[] {
...
export function SalesCreditNoteEditor({
  ...
  existing?: SalesCreditNoteDetail | null;
  /** "Duplicate credit note" prefill — create mode only. sales_invoice_id is
   * deliberately NOT copied (see lib/sales/salesCreditNoteDuplicateSeed.ts). */
  duplicateSeed?: SalesCreditNoteDetail | null;
  ...
  const [cnDate, setCnDate] = useState(existing?.credit_note_date ?? today);
  ...
  // credit_notes.py's update_credit_note enforces this identically).
  ...
  // SalesDebitNoteEditor.tsx (detailLinesToEditorLines only keeps the id).
  useEffect(() => {
  ...
  const initialSnapshot = useRef({
    ...
    cnDate: existing?.credit_note_date ?? today,
  ...
  const currentSnapshot = { customerId, cnDate, reason, notes, invoiceId, isInterstate, lines };
  ...
  // backend applies no round-off to a credit note, unlike a Sales Invoice).
  const totals = previewTotals(lines, isInterstate, false);
  const validation = validateSalesCreditNoteEditor({ customerId, creditNoteDate: cnDate, lines });
  const selectedInvoice = openInvoices.find((i) => i.id === invoiceId);
  const exceedsOutstanding = !!selectedInvoice && totals.grand_total_paise > invoiceOutstanding(selectedInvoice);

  function onCustomerChange(id: string) {
    if (isEdit) return; // customer is part of the note's identity once created — same lock rationale as Sales Debit Note
    setCustomerId(id);
    setInvoiceId("");
    loadOpenInvoices(id);
  }
  ...
  async function save() {
    ...
    try {
      ...
      if (isEdit && existing) {
        ...
        const upd = await apiCall(`/api/credit-notes/${existing.id}`, "PATCH", patchPayload, token);
        if (!upd.success) throw new Error(upd.error ?? "Failed to update credit note");
      } else {
      ...
      onDone(isEdit ? "Credit note updated" : "Credit note saved as draft");
    } catch (err) {
        ...
        <section className="bg-white rounded-xl border border-[#F1F5F9] p-4">
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
            <div>
              ...
              {isEdit && <p className="mt-1 text-[10px] text-[#94A3B8]">Customer can&apos;t be changed once a credit note exists.</p>}
            ...
            <div>
              ...
              {isLocked && <p className="mt-1 text-[10px] text-[#94A3B8]">Frozen once issued — issue a fresh credit note to correct (CGST Act §34).</p>}
            </div>
            <div>
