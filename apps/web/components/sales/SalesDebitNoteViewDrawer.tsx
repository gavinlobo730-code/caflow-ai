...
/**
 * SalesDebitNoteViewDrawer — read-only detail view + status-gated action bar
 * for a Sales Debit Note, mirroring DebitNoteViewDrawer's shape (purchase
 * side). No Cancel/reversal action here (deliberately absent platform-wide
 * for notes — CGST Act §34: correct with a fresh note, not a reversal), no
 * RCM chip (doesn't apply sales-side) and no attachment section (the Sales
 * Invoice baseline has none either).
 */
...
export function SalesDebitNoteViewDrawer({
  sdnId, clientId, customerName, onClose, onEdit, onIssue, onDelete, onDuplicate,
}: {
  sdnId: string;
  clientId: string;
  ...
  customerName: string;
  onClose: () => void;
  onEdit: (sdnId: string) => void;
  /** "Issue" — the caller POSTs /sales-debit-notes/{id}/issue and reloads its list. */
  onIssue: (dn: SalesDebitNoteDetail) => void;
  onDelete: (dn: SalesDebitNoteDetail) => void;
  /** "Duplicate" — hands the full loaded note to the caller, which stashes it
   * via lib/sales/salesDebitNoteDuplicateSeed and opens the New Debit Note route. */
  onDuplicate: (dn: SalesDebitNoteDetail) => void;
}) {
  const [dn, setDn] = useState<SalesDebitNoteDetail | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [d, tl] = await Promise.all([
        apiGet(`/api/sales-debit-notes/${sdnId}`, token),
      ...
      const parsed = objectOrNull<SalesDebitNoteDetail>(d.data);
      setDn(parsed ? { ...parsed, lines: arrayOrEmpty<SalesDebitNoteDetail["lines"][number]>(parsed.lines) } : null);
      const events = (tl.data as TimelineEvent[]) ?? [];
      const items: ActivityItem[] = events
        .filter((e) => e.entity_type === "sales_debit_note" && e.entity_id === sdnId)
          ...
          <section className="space-y-2.5">
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="text-right flex-shrink-0">
                ...
                <p className="text-3xs text-ps-hint">Debit note total</p>
              </div>
            </div>

            <div className="flex flex-wrap items-center gap-1.5">
              <span className={`px-2 py-0.5 rounded-full text-3xs font-medium ${DN_STATUS_BADGE[dn.status] ?? "bg-ps-muted text-ps-label"}`}>
                {dn.status}
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 pt-1">
              ...
              <DetailRow label="Applied to invoice" value={dn.sales_invoice_id ? "Yes" : "Standalone"} />
            ...
            {showJournal && (
              <div className="border border-ps-muted rounded-lg p-2 bg-ps-bg">
                {journalLoading ? (
                  ...
                  <p className="text-2xs text-ps-hint py-1">Journal {dn.journal_entry_id} — line detail unavailable here.</p>
                )}
              </div>
            )}
          </section>
