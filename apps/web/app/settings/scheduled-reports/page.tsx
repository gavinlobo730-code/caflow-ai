...
export default function ScheduledReportsPage() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400 mt-0.5">Automatically email reports to clients on a schedule</p>
          </div>
        </div>
        <button
      ...
      {/* List */}
      {loading ? (
        <div className="text-center py-12 flex items-center justify-center gap-2 text-gray-400">
          <Loader2 className="w-4 h-4 animate-spin" />
          <span className="text-sm">Loading schedules…</span>
        </div>
      ) : schedules.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-100 px-5 py-14 text-center">
          <Calendar className="w-8 h-8 text-gray-200 mx-auto mb-2" />
          <p className="text-sm text-gray-400">No schedules yet</p>
          <p className="text-xs text-gray-300 mt-1">Create your first scheduled report to get started</p>
        </div>
      ) : (
              ...
              {schedules.map(s => (
                <tr key={s.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-500">
                    {s.client_name ?? <span className="text-gray-300 italic">All clients</span>}
                  </td>
                  <td className="px-4 py-3 text-xs text-gray-600">
                    {FREQUENCY_LABELS[s.frequency]}
                  </td>
                  <td className="px-4 py-3 text-xs text-gray-600">{s.day_of_month}</td>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Client (optional — leave blank for all)</label>
                <select
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Day of month (1–28) *</label>
                <input
                  type="number"
                  min={1}
                  max={28}
