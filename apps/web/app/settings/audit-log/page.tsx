...
function actionBadgeClass(action: string): string {
  ...
  if (["delete", "platform_delete", "reject", "signature_failed"].includes(a)) return "bg-state-problem-surface text-state-problem";
  ...
  if (a.includes("status") || a.includes("review") || a.includes("send") || a.includes("reminder")) return "bg-state-attention-surface text-state-attention";
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={() => loadData()}
          disabled={loading}
          className="flex items-center gap-1.5 text-xs border border-ps-border text-ps-label px-3 py-1.5 rounded-md hover:bg-ps-bg disabled:opacity-40"
        >
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-xs border border-ps-border text-ps-label px-3 py-1.5 rounded-md hover:bg-ps-bg disabled:opacity-40 disabled:cursor-not-allowed"
        >
      ...
      <div className="bg-white border border-ps-muted rounded-xl px-5 py-4 grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <div>
          <label className="text-xs text-ps-label">
            User (email)
            <span className="ml-1 text-ps-disabled" title="Searches the events already loaded, not the whole log — the server matches a user by id, not by an email fragment.">
              · loaded only
            </span>
          </label>
      ...
      <div className="bg-white border border-ps-muted rounded-xl overflow-hidden">
        ...
        {!loading && error && (
          <div className="px-5 py-8 text-center space-y-3">
            ...
            <button onClick={() => loadData()} className="inline-flex items-center gap-1.5 text-xs border border-ps-border text-ps-label px-3 py-1.5 rounded-md hover:bg-ps-bg">
              <RefreshCw size={13} /> Retry
            </button>
          </div>
        )}
        {!loading && !error && rows.length === 0 && (
          <div className="px-5 py-12 text-center text-sm text-ps-hint">
          ...
          {filtered.map((row) => (
            <div key={row.id} className="grid grid-cols-12 gap-2 px-5 py-3 hover:bg-ps-bg transition-colors items-start text-xs">
              <span className="col-span-2 text-ps-label tabular-nums">
                ...
                <span className="block text-ps-hint font-mono">{row.created_at.slice(11, 19)}</span>
              </span>
              <span className="col-span-2 text-ps-body truncate" title={row.actor}>{row.actor}</span>
              <span className="col-span-2">
                <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${actionBadgeClass(row.action)}`}>
                  {formatAction(row.action)}
                </span>
              </span>
              <span className="col-span-2 text-ps-label">{formatEntityType(row.entity_type)}</span>
