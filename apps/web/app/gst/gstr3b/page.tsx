  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white/85">GSTR-3B Review</h1>
      ...
      {result && w && (
        <>
          ...
          <section className="bg-[#131620] border border-white/[0.07] rounded-xl overflow-hidden">
            <div className="px-5 py-3 bg-[#0e1017] border-b border-white/[0.07]">
              <h3 className="font-semibold text-white/75 text-sm">
                Table 3.1 — Outward Taxable Supplies
              </h3>
              <p className="text-xs text-white/40 mt-0.5">Net of credit notes. CGST Act Section 37.</p>
            </div>
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-white/40 uppercase border-b border-white/[0.05]">
              ...
              <tbody className="divide-y divide-white/[0.03]">
                <tr className="hover:bg-[#0e1017]">
                  <td className="px-5 py-3 text-white/65">Taxable supplies (B2B + B2C + B2CL)</td>
                  <td className="px-5 py-3 text-right font-mono text-white/85">{r(w.outward.taxable_igst_paise)}</td>
                  <td className="px-5 py-3 text-right font-mono text-white/85">{r(w.outward.taxable_cgst_paise)}</td>
                  <td className="px-5 py-3 text-right font-mono text-white/85">{r(w.outward.taxable_sgst_paise)}</td>
                </tr>
                <tr className="hover:bg-[#0e1017]">
                  <td className="px-5 py-3 text-white/65">Zero-rated supplies (Exports / SEZ)</td>
                  <td className="px-5 py-3 text-right font-mono text-white/40">{r(w.outward.zero_rated_paise)}</td>
                  <td className="px-5 py-3 text-right font-mono text-white/30">—</td>
                  <td className="px-5 py-3 text-right font-mono text-white/30">—</td>
                </tr>
                <tr className="hover:bg-[#0e1017]">
                  <td className="px-5 py-3 text-white/65">Nil-rated / Exempt</td>
                  <td className="px-5 py-3 text-right font-mono text-white/40">{r(w.outward.nil_exempt_paise)}</td>
                  <td className="px-5 py-3 text-right font-mono text-white/30">—</td>
                  <td className="px-5 py-3 text-right font-mono text-white/30">—</td>
                </tr>
          ...
          <section className="bg-[#131620] border border-white/[0.07] rounded-xl overflow-hidden">
            <div className="px-5 py-3 bg-[#0e1017] border-b border-white/[0.07] flex items-center justify-between">
              <div>
                <h3 className="font-semibold text-white/75 text-sm">Table 4 — ITC Available</h3>
                <p className="text-xs text-white/40 mt-0.5">CGST Rule 36(4): capped at 105% of GSTR-2A credit.</p>
              </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-white/[0.03]">
                <tr className="hover:bg-[#0e1017]">
                  <td className="px-5 py-3 text-white/55 text-xs">As per books (purchase register)</td>
                  <td className="px-5 py-3 text-right font-mono text-white/40 text-xs">{r(w.itc.book_igst_paise)}</td>
                  <td className="px-5 py-3 text-right font-mono text-white/40 text-xs">{r(w.itc.book_cgst_paise)}</td>
                  <td className="px-5 py-3 text-right font-mono text-white/40 text-xs">{r(w.itc.book_sgst_paise)}</td>
                </tr>
                <tr className="hover:bg-[#0e1017]">
                  <td className="px-5 py-3 text-white/55 text-xs">As per GSTR-2A (supplier-filed)</td>
                  <td className="px-5 py-3 text-right font-mono text-white/40 text-xs">{r(w.itc.gstr2a_igst_paise)}</td>
                  <td className="px-5 py-3 text-right font-mono text-white/40 text-xs">{r(w.itc.gstr2a_cgst_paise)}</td>
                  <td className="px-5 py-3 text-right font-mono text-white/40 text-xs">{r(w.itc.gstr2a_sgst_paise)}</td>
                </tr>
          ...
          <section className="bg-[#131620] border border-white/[0.07] rounded-xl overflow-hidden">
            <div className="px-5 py-3 bg-[#0e1017] border-b border-white/[0.07]">
              <h3 className="font-semibold text-white/75 text-sm">Table 6 — Net Tax Payable</h3>
              <p className="text-xs text-white/40 mt-0.5">CGST Act Section 49: IGST ITC cross-utilised against CGST/SGST if excess.</p>
            </div>
            <div className="grid grid-cols-4 divide-x divide-white/[0.05] text-center">
              {[
                { label: "IGST", value: w.net_payable.igst_paise, color: "text-blue-700" },
                { label: "CGST", value: w.net_payable.cgst_paise, color: "text-blue-400" },
                ...
                <div key={item.label} className="px-4 py-5">
                  <p className="text-xs text-white/40 font-medium mb-1">{item.label}</p>
          ...
          <section className="bg-[#131620] border border-white/[0.07] rounded-xl overflow-hidden">
            <div className="px-5 py-3 bg-[#0e1017] border-b border-white/[0.07]">
              <h3 className="font-semibold text-white/75 text-sm">GSTN Payload Preview</h3>
              <p className="text-xs text-white/40 mt-0.5">
                This is the JSON that will be uploaded to gst.gov.in after CA approval.
              </p>
            </div>
            <pre className="p-5 text-xs font-mono text-white/55 overflow-auto max-h-80 bg-[#0e1017]">
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-[#131620] rounded-xl shadow-xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-white/85">Mark GSTR-3B as Filed</h3>
