  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg md:text-xl font-semibold text-gray-900">Section 80 Deductions Planner</h1>
          <p className="text-sm text-gray-500 mt-0.5">IT Act — Compute deductions and compare tax regimes</p>
        </div>
      ...
      <SectionCard title="Section 80D — Health Insurance" eligible={elig80d}>
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-sm text-gray-700">Self + Family Premium</span>
            <div className="flex items-center gap-3">
          ...
          <div className="flex items-center justify-between">
            <span className="text-sm text-gray-700">Parents Premium</span>
            <label className="flex items-center gap-1 text-xs text-gray-500">
          ...
          {state.donations.map((d, i) => (
            <div key={i} className="flex gap-2 items-center">
              <input type="text" value={d.description}
                ...
                className="flex-1 border border-gray-200 rounded px-2 py-1 text-sm outline-none focus:border-blue-500" />
              <input type="number" min="0" value={d.amountPaise / 100}
                ...
                placeholder="₹"
                className="w-24 border border-gray-200 rounded px-2 py-1 text-sm outline-none focus:border-blue-500" />
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
        <h2 className="font-semibold text-gray-900 text-sm">Tax Summary</h2>
        <div className="space-y-2 text-sm">
          <div className="flex justify-between"><span className="text-gray-600">Gross Total Income</span><span className="font-mono">{fmtP(state.grossIncomePaise)}</span></div>
          <div className="flex justify-between"><span className="text-gray-600">Total Deductions (Old Regime)</span><span className="font-mono text-green-600">— {fmtP(totalDeductions)}</span></div>
        ...
        <div className="grid grid-cols-2 gap-3 mt-3">
          <div className={`rounded-lg p-3 border-2 ${recommended === "new" ? "border-green-500 bg-green-50" : "border-gray-200 bg-gray-50"}`}>
            <p className="text-xs font-medium text-gray-600 mb-1">New Regime Tax {recommended === "new" && "✓ Recommended"}</p>
            <p className="text-xl font-bold font-mono text-gray-900">{fmtP(newRegimeTax)}</p>
            <p className="text-xs text-gray-500 mt-1">Taxable: {fmtP(netTaxableNew)}</p>
            {netTaxableNew <= 1200 * 100 * 100 && <p className="text-xs text-green-600 mt-1">Rebate u/s 87A — NIL tax</p>}
          </div>
          <div className={`rounded-lg p-3 border-2 ${recommended === "old" ? "border-green-500 bg-green-50" : "border-gray-200 bg-gray-50"}`}>
            <p className="text-xs font-medium text-gray-600 mb-1">Old Regime Tax {recommended === "old" && "✓ Recommended"}</p>
            <p className="text-xl font-bold font-mono text-gray-900">{fmtP(oldRegimeTax)}</p>
            <p className="text-xs text-gray-500 mt-1">Taxable: {fmtP(netTaxableOld)}</p>
          </div>
        </div>
