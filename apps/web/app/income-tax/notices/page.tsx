    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h3 className="text-sm font-semibold text-gray-900">Add IT Notice</h3>
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-semibold text-gray-900">IT Notices Tracker</h1>
          <p className="text-sm text-gray-500 mt-0.5">Sections 143(1), 148, 271 — Income Tax notices with document upload</p>
        </div>
            ...
            <table className="w-full text-sm min-w-[1000px]">
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map(n => {
                  ...
                  return (
                    <tr key={n.id} className={`hover:bg-gray-50 ${overdue ? "bg-red-50/40" : ""}`}>
                      <td className="px-5 py-3 text-sm font-medium text-gray-900">{clientName(n.client_id)}</td>
                      <td className="px-3 py-3 text-sm">Sec. {n.notice_type}</td>
                      <td className="px-3 py-3 text-xs text-gray-600">{n.assessment_year}</td>
                      <td className="px-3 py-3 text-xs text-gray-600">{n.date_received}</td>
                      <td className={`px-3 py-3 text-xs ${overdue ? "text-red-600 font-semibold" : "text-gray-600"}`}>
                        {n.response_due_date ?? "—"}
                        {overdue && " ⚠"}
                      </td>
