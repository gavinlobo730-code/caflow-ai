...
function StatusBadge({ status }: { status: AISLine["status"] }) {
  ...
  if (status === "matched")
    return <span className={`${base} bg-state-ready-surface text-state-ready`}><CheckCircle2 className="w-3 h-3" /> Agreed</span>;
  if (status === "explained")
...
export default function AISPage() {
  ...
  return (
    <div className="p-6 max-w-ps-data mx-auto space-y-6">
      ...
      {message && !error && (
        <div className="bg-state-ready-surface border border-state-ready-border rounded-xl px-4 py-3 flex items-start gap-2">
          <CheckCircle2 className="w-4 h-4 text-state-ready shrink-0 mt-0.5" />
          <p className="text-xs text-state-ready">{message}</p>
        </div>
      )}
                    ...
                    <tr key={line.id} className="hover:bg-ps-bg/50 transition-colors align-top">
                      <td className="px-5 py-3">
                        ...
                        <span className={`mt-1 inline-block text-3xs px-1.5 py-0.5 rounded-full font-medium ${
                          line.source === "json" ? "bg-brand-surface text-brand"
                      ...
                      <td className="px-4 py-3 text-right text-xs font-medium">
                        {diff === null ? <span className="text-ps-disabled">—</span>
                          : diff === 0 ? <span className="text-state-ready">₹0.00</span>
                          : <span className="text-state-attention">{formatRupees(diff)}</span>}
                      </td>
        ...
        <div className="bg-white rounded-xl border border-ps-muted overflow-hidden">
          ...
          <div className="px-5 py-5 grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-state-problem-surface rounded-xl p-4">
              <p className="text-xs text-state-problem font-medium uppercase tracking-wide mb-1">Not in the books</p>
            ...
            <div className="bg-state-attention-surface rounded-xl p-4">
              <p className="text-xs text-state-attention font-medium uppercase tracking-wide mb-1">Books short by</p>
          ...
          <div className="px-5 pb-5">
            <div className="bg-brand-surface border border-brand-light rounded-xl px-4 py-3 flex items-start gap-2">
              <HelpCircle className="w-4 h-4 text-brand shrink-0 mt-0.5" />
              <p className="text-xs text-brand">{summary.tax_impact_refused}</p>
            </div>
          </div>
          ...
          {summary.not_reviewed_count === 0 && summary.open_paise === 0 && (
            <div className="px-5 pb-5">
              <div className="bg-state-ready-surface border border-state-ready-border rounded-xl px-4 py-3 flex items-start gap-3">
                <CheckCircle2 className="w-4 h-4 text-state-ready shrink-0 mt-0.5" />
      ...
      <div className="bg-brand-surface border border-brand-light rounded-xl px-5 py-3">
