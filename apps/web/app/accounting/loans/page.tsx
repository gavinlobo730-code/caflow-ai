    ...
    <div className="p-6 max-w-ps-data mx-auto space-y-6">
      ...
      <div className="flex gap-1 bg-ps-muted rounded-xl p-1 w-fit">
        <button
          ...
          className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${tab === "loans" ? "bg-white text-ps-ink shadow-sm" : "text-ps-label hover:text-ps-body"}`}
        >
          Loans
        </button>
        <button
          onClick={() => setTab("fd")}
          className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${tab === "fd" ? "bg-white text-ps-ink shadow-sm" : "text-ps-label hover:text-ps-body"}`}
        >
          Fixed Deposits
        </button>
      </div>
      ...
      {tab === "loans" && (
        <>
          ...
          <div className="bg-white rounded-xl border border-ps-muted overflow-hidden">
            ...
            {loading ? (
              ...
              <div className="px-5 py-12 text-center space-y-2">
                ...
                <p className="text-sm text-ps-label">No loans recorded yet</p>
                <button
                  ...
                  <tbody className="divide-y divide-ps-bg">
                    {loans.map((loan) => (
                      <tr key={loan.id} className="hover:bg-ps-bg/50">
                        ...
                        <td className="px-4 py-3 text-right text-xs tabular-nums text-ps-body">
                          {formatPaise(loan.principal_paise)}
                        </td>
                        <td className="px-4 py-3 text-right text-xs tabular-nums font-medium text-ps-ink">
                          {formatPaise(loan.outstanding_paise)}
                        </td>
                        <td className="px-4 py-3 text-right text-xs text-ps-label">
          ...
          <div className="bg-white rounded-xl border border-ps-muted p-5">
            ...
            <p className="text-xs text-ps-hint mb-4">
              Formula: EMI = P × r × (1+r)^n / ((1+r)^n − 1) where r = monthly rate (annual rate ÷ 12 ÷ 100)
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-medium text-ps-body mb-1.5">Principal (₹)</label>
                <input
                  type="number"
                  min="0"
                  step="1000"
              ...
              <div>
                <label className="block text-xs font-medium text-ps-body mb-1.5">Interest Rate (% p.a.)</label>
                <input
                  type="number"
                  min="0"
                  step="0.01"
              ...
              <div>
                <label className="block text-xs font-medium text-ps-body mb-1.5">Tenure (months)</label>
                <input
                  type="number"
                  min="1"
                  step="1"
          ...
          <div className="bg-white rounded-xl border border-ps-muted overflow-hidden">
            ...
            {loading ? (
              ...
              <div className="px-5 py-12 text-center space-y-2">
                ...
                <p className="text-sm text-ps-label">No fixed deposits recorded yet</p>
                <button
                    ...
                    {fds.map((fd) => {
                      ...
                      return (
                        <tr key={fd.id} className={`hover:bg-ps-bg/50 ${isMaturingSoon ? "bg-state-attention-surface/30" : ""}`}>
                          ...
                          <td className="px-4 py-3 text-xs text-ps-label">{formatDate(fd.start_date)}</td>
                          <td className="px-4 py-3 text-xs text-ps-label">{formatDate(fd.maturity_date)}</td>
                          <td className="px-4 py-3 text-right text-xs tabular-nums font-medium text-ps-ink">
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="rounded"
                  />
                  <span className="text-xs font-medium text-ps-body">Auto Renewal</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    ...
                    className="rounded"
                  />
                  <span className="text-xs font-medium text-ps-body">TDS Applicable (s.194A)</span>
                </label>
              </div>
...
function SummaryCard({
  ...
  return (
    <div className="bg-white rounded-xl border border-ps-muted p-4">
      ...
      <p className="text-lg font-semibold text-ps-ink">{value}</p>
      <p className="text-xs text-ps-hint mt-0.5">{sub}</p>
    </div>
  );
}
...
function FDStatusBadge({ status }: { status: FDStatus }) {
  const styles: Record<FDStatus, string> = {
    active: "bg-state-ready-surface text-state-ready",
    matured: "bg-state-done-surface text-state-done",
