...
const inputCls =
  "w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white";
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-3">
          <Link href="/accounting" className="text-gray-400 hover:text-gray-600 mt-0.5">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <div>
            <h1 className="text-xl font-semibold text-gray-900">Loans & Fixed Deposits</h1>
            <p className="text-sm text-gray-500 mt-0.5">Track client borrowings and FD investments</p>
          </div>
        </div>
        <button
      ...
      <div className="flex gap-1 bg-gray-100 rounded-xl p-1 w-fit">
        <button
          ...
          className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${tab === "loans" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
        >
          Loans
        </button>
        <button
          onClick={() => setTab("fd")}
          className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${tab === "fd" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
        >
          Fixed Deposits
        </button>
      </div>
        ...
        <>
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-50">
              <h2 className="text-sm font-semibold text-gray-900">Loan Register</h2>
            </div>
            {loading ? (
              <div className="px-5 py-12 text-center text-sm text-gray-400 animate-pulse">Loading…</div>
            ) : loans.length === 0 ? (
              <div className="px-5 py-12 text-center space-y-2">
                <TrendingDown className="w-10 h-10 text-gray-200 mx-auto" />
                <p className="text-sm text-gray-500">No loans recorded yet</p>
                <button
                    ...
                    {loans.map((loan) => (
                      <tr key={loan.id} className="hover:bg-gray-50/50">
                        <td className="px-5 py-3">
                          ...
                          {loan.account_number && (
                            <p className="text-[10px] font-mono text-gray-400">{loan.account_number}</p>
                          )}
                        </td>
                        ...
                        <td className="px-4 py-3 text-right text-xs tabular-nums font-medium text-gray-900">
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-sm font-semibold text-gray-900">EMI Calculator</h2>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1.5">Principal (₹)</label>
                <input
                  type="number"
                  min="0"
                  step="1000"
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1.5">Interest Rate (% p.a.)</label>
                <input
                  type="number"
                  min="0"
                  step="0.01"
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1.5">Tenure (months)</label>
                <input
                  type="number"
                  min="1"
                  step="1"
      ...
      {tab === "fd" && (
        <>
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-50">
              <h2 className="text-sm font-semibold text-gray-900">FD Register</h2>
            ...
            {loading ? (
              ...
              <div className="px-5 py-12 text-center space-y-2">
                ...
                <p className="text-sm text-gray-500">No fixed deposits recorded yet</p>
                <button
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {fds.map((fd) => {
                      ...
                      return (
                        <tr key={fd.id} className={`hover:bg-gray-50/50 ${isMaturingSoon ? "bg-amber-50/30" : ""}`}>
                          <td className="px-5 py-3 font-medium text-gray-900">{fd.bank_name}</td>
                          <td className="px-4 py-3 text-xs text-gray-600">{fd.clients?.name ?? "—"}</td>
                          <td className="px-4 py-3 text-xs font-mono text-gray-600">{fd.fd_number ?? "—"}</td>
                          <td className="px-4 py-3 text-right text-xs tabular-nums text-gray-700">
                          ...
                          <td className="px-4 py-3 text-xs text-gray-600">{formatDate(fd.start_date)}</td>
                          <td className="px-4 py-3 text-xs text-gray-600">{formatDate(fd.maturity_date)}</td>
                          <td className="px-4 py-3 text-right text-xs tabular-nums font-medium text-gray-900">
                          ...
                          <td className="px-4 py-3 text-right text-xs tabular-nums">
                            {fd.status !== "active" ? (
                              ...
                              <span className={isMaturingSoon ? "text-amber-600 font-medium" : "text-gray-600"}>
                                {days}d
                              </span>
                            )}
                          </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
              <h3 className="text-base font-semibold text-gray-900">Add Loan</h3>
              <button onClick={() => { setShowAddLoan(false); resetLoanForm(); }} className="text-gray-400 hover:text-gray-600">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex gap-3 justify-end">
              <button onClick={() => { setShowAddLoan(false); resetLoanForm(); }} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200">Cancel</button>
      ...
      {showAddFD && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
              <h3 className="text-base font-semibold text-gray-900">Add Fixed Deposit</h3>
              <button onClick={() => { setShowAddFD(false); resetFDForm(); }} className="text-gray-400 hover:text-gray-600">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="flex gap-4">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="rounded"
                  />
                  <span className="text-xs font-medium text-gray-700">TDS Applicable (s.194A)</span>
                </label>
              </div>
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex gap-3 justify-end">
              <button onClick={() => { setShowAddFD(false); resetFDForm(); }} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200">Cancel</button>
