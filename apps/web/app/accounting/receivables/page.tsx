    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500 mt-0.5">Outstanding invoices by age bucket</p>
        </div>
        {generated && (
      ...
      {generated && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: "Total Outstanding", value: fmtRs(totalOutstanding), cls: "text-gray-900" },
              ...
              <tbody className="divide-y divide-gray-50">
                {rows.map(r => (
                  <tr key={r.invoice.id} className={ROW_COLOR[r.bucket]}>
                    <td className="px-4 py-3 font-mono text-xs font-medium text-gray-900">{r.invoice.invoice_no}</td>
                    <td className="px-4 py-3 text-gray-700">{r.clientName}</td>
                    <td className="px-4 py-3 text-gray-600">{r.invoice.invoice_date}</td>
                    <td className="px-4 py-3 text-gray-600">{r.invoice.due_date ?? "—"}</td>
                    <td className="px-4 py-3 text-right text-gray-700">{fmtRs(r.invoice.total_paise)}</td>
                    <td className="px-4 py-3 text-right font-semibold text-gray-900">{fmtRs(r.outstandingPaise)}</td>
                    <td className="px-4 py-3 text-right text-gray-600">{r.daysOverdue > 0 ? r.daysOverdue : "—"}</td>
                ...
                {rows.length === 0 && (
                  <tr><td colSpan={9} className="px-4 py-8 text-center text-gray-400 text-sm">No outstanding invoices found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {!generated && !loading && (
        <div className="bg-white rounded-xl border border-gray-100 p-12 text-center">
          ...
          <p className="text-sm text-gray-400">Select a client and click &quot;Generate Aging&quot; to see outstanding invoices</p>
        </div>
      )}
    </div>
  );
