  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-semibold text-[#0F172A]">Accounting</h1>
          <p className="text-sm text-[#64748B] mt-0.5">Double-entry books — FY 2025-26</p>
        </div>
      ...
      <Card>
        ...
        {loading ? (
          <div className="px-5 py-6 text-sm text-[#94A3B8] text-center animate-pulse">Loading…</div>
        ) : (
          <div className="divide-y divide-[#F8FAFC]">
            {entries.map((entry) => (
              <div key={entry.id} className="flex items-center gap-4 px-5 py-3.5">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-[#0F172A] truncate">{entry.narration}</p>
                  <p className="text-xs text-[#94A3B8] mt-0.5">{formatDate(entry.entry_date)} · {entry.reference_no} · {entry.entry_type}</p>
                </div>
            ...
            {entries.length === 0 && (
              <div className="px-5 py-6 text-sm text-[#94A3B8] text-center">No entries yet</div>
            )}
          </div>
        )}
      </Card>
