...
function SetRetainerModal({ client, existing, onSave, onClose }: SetRetainerModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-[#131620] rounded-xl shadow-xl w-full max-w-md p-6 space-y-5 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-white/85">Set Retainer — {client.client_name}</h3>
        ...
        {/* Monthly Fee */}
        <div>
          <label className="text-xs font-medium text-white/65 block mb-1">Monthly Fee (₹)</label>
          <div className="relative">
            ...
            <input
              ...
              min="0"
              step="1"
              className="w-full border border-white/[0.07] rounded-lg pl-7 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              placeholder="15000"
              value={feeRupees}
              onChange={e => setFeeRupees(e.target.value)}
            />
          </div>
          <p className="text-[10px] text-white/30 mt-1">Stored as paise internally — integer arithmetic</p>
        </div>

        {/* Services */}
        <div>
          <label className="text-xs font-medium text-white/65 block mb-2">Services Included</label>
          <div className="grid grid-cols-2 gap-2">
            {SERVICES.map(s => (
              <label key={s} className="flex items-center gap-2 text-xs text-white/65 cursor-pointer">
                <input
                  type="checkbox"
        ...
        {/* Invoice Day */}
        <div>
          <label className="text-xs font-medium text-white/65 block mb-1">Invoice Day of Month</label>
          <select
        ...
        <div className="flex gap-2 pt-1">
          <button onClick={onClose} className="flex-1 border border-white/[0.07] text-white/55 text-sm py-2 rounded-lg hover:bg-[#0e1017]">
            Cancel
          </button>
          <button
      ...
      <div className="bg-[#131620] rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div id="invoice-print-area" className="p-8 space-y-5">
          ...
          <div className="flex items-start justify-between border-b border-white/[0.07] pb-5">
            <div>
              <h2 className="text-lg font-bold text-white/85">{firmName}</h2>
              {firmGstin && <p className="text-xs text-white/40 mt-0.5">GSTIN: {firmGstin}</p>}
            </div>
            <div className="text-right">
              <p className="text-base font-semibold text-blue-700">TAX INVOICE</p>
              <p className="text-xs text-white/40 mt-0.5">No: {invoiceNo}</p>
              <p className="text-xs text-white/40">Date: {dateStr}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 text-xs">
            <div>
              <p className="font-semibold text-white/65 mb-1">Bill To</p>
              <p className="text-white/85 font-medium">{client.client_name}</p>
              {client.gstin && <p className="text-white/40 font-mono">GSTIN: {client.gstin}</p>}
              {client.pan && <p className="text-white/40 font-mono">PAN: {client.pan}</p>}
              {client.address_line1 && <p className="text-white/40">{client.address_line1}</p>}
              {client.city && <p className="text-white/40">{client.city}{client.state ? `, ${client.state}` : ""}</p>}
            </div>
            <div className="text-right">
              <p className="font-semibold text-white/65 mb-1">Invoice Details</p>
              <p className="text-white/55">Month: {today.toLocaleDateString("en-IN", { month: "long", year: "numeric" })}</p>
            </div>
          </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-[#0e1017] rounded">
                <th className="text-left font-semibold text-white/55 px-3 py-2">Description</th>
                <th className="text-right font-semibold text-white/55 px-3 py-2">Amount</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/[0.05]">
              <tr>
                <td className="px-3 py-2">
                  <p className="font-medium text-white/85">Monthly Retainer Fee</p>
                ...
                <td className="px-3 py-2 text-right font-medium text-white/85">{formatPaise(config.feePaise)}</td>
              </tr>
            </tbody>
          </table>

          {/* GST breakdown — CGST Act Section 9, Notification 11/2017-CT(Rate), 18% on CA services */}
          <div className="border-t border-white/[0.05] pt-3 space-y-1.5 text-xs">
            ...
            <div className="flex justify-between font-bold text-white/85 text-sm border-t border-white/[0.07] pt-2 mt-2">
              <span>Total</span>
              <span>{formatPaise(firmGstin ? grandTotal : config.feePaise)}</span>
            </div>
          </div>

          <p className="text-[10px] text-white/30 text-center border-t border-white/[0.05] pt-3">
        ...
        <div className="px-6 pb-6 flex gap-2 print:hidden">
          ...
          <button
            onClick={handlePrint}
            className="flex items-center gap-1.5 px-4 border border-white/[0.07] text-white/65 text-sm py-2 rounded-lg hover:bg-[#0e1017]"
          >
...
export default function RetainerPage() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-white/85">Monthly Retainer Tracker</h1>
        <p className="text-sm text-white/40 mt-0.5">Track fixed-fee retainer clients, work done, and generate invoices</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="bg-[#131620] rounded-xl border border-white/[0.05] p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-white/40">Retainer Clients</span>
          </div>
          <p className="text-lg font-semibold text-white/85">{loading ? "—" : retainerClients.length}</p>
          <p className="text-xs text-white/30 mt-0.5">of {clients.length} total clients</p>
        </div>
        ...
        <div className="bg-[#131620] rounded-xl border border-white/[0.05] p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-white/40">Monthly Revenue</span>
          </div>
          <p className="text-lg font-semibold text-white/85">{loading ? "—" : formatPaise(totalRevenuePaise)}</p>
          <p className="text-xs text-white/30 mt-0.5">Sum of all retainer fees</p>
        </div>
        ...
        <div className="bg-[#131620] rounded-xl border border-white/[0.05] p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-white/40">Invoiced This Month</span>
          </div>
          <p className="text-lg font-semibold text-white/85">{invoicesThisMonth}</p>
          <p className="text-xs text-white/30 mt-0.5">{monthKey}</p>
        </div>
        ...
        <div className="bg-[#131620] rounded-xl border border-white/[0.05] p-4">
          ...
          <p className="text-lg font-semibold text-white/85">{outstandingCount}</p>
          <p className="text-xs text-white/30 mt-0.5">Not yet invoiced</p>
        </div>
      </div>
      ...
      <div className="bg-[#131620] rounded-xl border border-white/[0.05] overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-50 flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-white/85">Retainer Clients</h2>
            <p className="text-xs text-white/30 mt-0.5">Click &quot;Set Retainer&quot; to configure a client</p>
          </div>
        </div>

        {loading ? (
          <div className="px-5 py-8 text-center text-sm text-white/30">Loading clients...</div>
        ) : clients.length === 0 ? (
          <div className="px-5 py-8 text-center text-sm text-white/30">No clients found — add clients first</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-50">
                  <th className="text-left text-xs font-medium text-white/30 px-5 py-3">Client Name</th>
                  <th className="text-right text-xs font-medium text-white/30 px-3 py-3">Monthly Fee</th>
                  <th className="text-left text-xs font-medium text-white/30 px-3 py-3">Services</th>
                  <th className="text-left text-xs font-medium text-white/30 px-3 py-3">Work Status</th>
                  <th className="text-left text-xs font-medium text-white/30 px-3 py-3">Invoice Status</th>
                  <th className="text-left text-xs font-medium text-white/30 px-5 py-3">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/[0.03]">
                {clients.map(client => {
                  ...
                  return (
                    <>
                      <tr key={client.id} className="hover:bg-[#0e1017]/50">
                        <td className="px-5 py-3">
                          <p className="text-sm font-medium text-white/85">{client.client_name}</p>
                          {client.gstin && (
                            <p className="text-[10px] font-mono text-white/30">{client.gstin}</p>
                          )}
                        </td>
                        <td className="px-3 py-3 text-right">
                          {config ? (
                            <span className="text-sm font-semibold text-white/85">{formatPaise(config.feePaise)}</span>
                          ) : (
                          ...
                          {config && config.services.length > 0 ? (
                            <div className="flex flex-wrap gap-1">
                              ...
                              {config.services.length > 2 && (
                                <span className="text-[10px] text-white/30">+{config.services.length - 2} more</span>
                              )}
                            </div>
                          ) : (
                        ...
                        <td className="px-3 py-3">
                          {config ? (
                            ...
                            >
                              <span className={`text-[10px] rounded-full px-1.5 py-0.5 font-medium ${completedCount === WORK_ITEMS.length ? "bg-green-100 text-green-700" : completedCount > 0 ? "bg-amber-100 text-amber-700" : "bg-white/[0.06] text-white/40"}`}>
                        ...
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            {config && isInvoiced && (
                              <button
                                onClick={() => setInvoiceClient(client)}
                                className="text-xs text-white/30 hover:text-white/55 font-medium whitespace-nowrap"
                              >
                                View Invoice
                              </button>
                            )}
                      ...
                      {isExpanded && config && (
                        <tr key={`${client.id}-work`} className="bg-blue-50/30">
                          <td colSpan={6} className="px-5 py-3">
                            <p className="text-[11px] font-semibold text-white/55 mb-2">Work Done This Month</p>
                            <div className="flex flex-wrap gap-3">
                              {WORK_ITEMS.map(item => (
                                <label key={item.key} className="flex items-center gap-2 text-xs text-white/65 cursor-pointer">
                                  <input
                                    type="checkbox"
      ...
      <div className="bg-[#131620] rounded-xl border border-white/[0.05] overflow-hidden">
        ...
        >
          <div>
            <h2 className="text-sm font-semibold text-white/85">Invoice History</h2>
            <p className="text-xs text-white/30 mt-0.5">{invoices.length} invoices generated</p>
          </div>
          {showHistory ? <ChevronUp className="w-4 h-4 text-white/30" /> : <ChevronDown className="w-4 h-4 text-white/30" />}
        </div>
        {showHistory && (
          invoices.length === 0 ? (
            <div className="px-5 py-6 text-sm text-white/30 text-center">No invoices generated yet</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-50">
                    <th className="text-left text-xs font-medium text-white/30 px-5 py-3">Invoice No</th>
                    <th className="text-left text-xs font-medium text-white/30 px-3 py-3">Client</th>
                    <th className="text-left text-xs font-medium text-white/30 px-3 py-3">Month</th>
                    <th className="text-right text-xs font-medium text-white/30 px-3 py-3">Amount</th>
                    <th className="text-right text-xs font-medium text-white/30 px-3 py-3">GST</th>
                    <th className="text-right text-xs font-medium text-white/30 px-5 py-3">Total</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/[0.03]">
                  {invoices.map(inv => (
                    <tr key={inv.id} className="hover:bg-[#0e1017]/50">
                      <td className="px-5 py-3 text-xs font-mono text-blue-700">{inv.invoiceNo}</td>
                      <td className="px-3 py-3 text-sm text-white/85">{inv.clientName}</td>
                      <td className="px-3 py-3 text-xs text-white/40">{inv.month}</td>
                      <td className="px-3 py-3 text-right text-sm text-white/85">{formatPaise(inv.amountPaise)}</td>
                      <td className="px-3 py-3 text-right text-xs text-white/40">{formatPaise(inv.gstPaise)}</td>
                      <td className="px-5 py-3 text-right text-sm font-semibold text-white/85">{formatPaise(inv.totalPaise)}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="border-t border-white/[0.05] bg-[#0e1017]/50">
                    <td colSpan={3} className="px-5 py-3 text-xs font-medium text-white/40">Total</td>
                    <td className="px-3 py-3 text-right text-sm font-semibold text-white/85">
                      {formatPaise(invoices.reduce((s, inv) => s + inv.amountPaise, 0))}
                    </td>
                    <td className="px-3 py-3 text-right text-xs font-semibold text-white/40">
                      {formatPaise(invoices.reduce((s, inv) => s + inv.gstPaise, 0))}
                    </td>
                    <td className="px-5 py-3 text-right text-sm font-semibold text-white/85">
