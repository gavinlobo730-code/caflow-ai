...
const LS_RETAINERS = "caflow_retainers";
const LS_INVOICES = "caflow_invoices";
const LS_WORK = "caflow_retainer_work";
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 space-y-5 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-gray-900">Set Retainer — {client.client_name}</h3>
        ...
        {/* Monthly Fee */}
        <div>
          <label className="text-xs font-medium text-gray-700 block mb-1">Monthly Fee (₹)</label>
          ...
          <p className="text-[10px] text-gray-400 mt-1">Stored as paise internally — integer arithmetic</p>
        </div>

        {/* Services */}
        <div>
          <label className="text-xs font-medium text-gray-700 block mb-2">Services Included</label>
        ...
        {/* Invoice Day */}
        <div>
          <label className="text-xs font-medium text-gray-700 block mb-1">Invoice Day of Month</label>
          <select
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div id="invoice-print-area" className="p-8 space-y-5">
          ...
          <div className="flex items-start justify-between border-b border-gray-200 pb-5">
            <div>
              <h2 className="text-lg font-bold text-gray-900">{firmName}</h2>
              {firmGstin && <p className="text-xs text-gray-500 mt-0.5">GSTIN: {firmGstin}</p>}
            </div>
            <div className="text-right">
              <p className="text-base font-semibold text-blue-700">TAX INVOICE</p>
              <p className="text-xs text-gray-500 mt-0.5">No: {invoiceNo}</p>
          ...
          <div className="grid grid-cols-2 gap-4 text-xs">
            <div>
              ...
              <p className="text-gray-900 font-medium">{client.client_name}</p>
              {client.gstin && <p className="text-gray-500 font-mono">GSTIN: {client.gstin}</p>}
              {client.pan && <p className="text-gray-500 font-mono">PAN: {client.pan}</p>}
              {client.address_line1 && <p className="text-gray-500">{client.address_line1}</p>}
              {client.city && <p className="text-gray-500">{client.city}{client.state ? `, ${client.state}` : ""}</p>}
            </div>
            <div className="text-right">
              <p className="font-semibold text-gray-700 mb-1">Invoice Details</p>
              <p className="text-gray-600">Month: {today.toLocaleDateString("en-IN", { month: "long", year: "numeric" })}</p>
            </div>
          </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-gray-50 rounded">
                ...
                <th className="text-right font-semibold text-gray-600 px-3 py-2">Amount</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              <tr>
                <td className="px-3 py-2">
                  <p className="font-medium text-gray-900">Monthly Retainer Fee</p>
                ...
                <td className="px-3 py-2 text-right font-medium text-gray-900">{formatPaise(config.feePaise)}</td>
              </tr>
            </tbody>
          </table>
          ...
          <div className="border-t border-gray-100 pt-3 space-y-1.5 text-xs">
            ...
            <div className="flex justify-between font-bold text-gray-900 text-sm border-t border-gray-200 pt-2 mt-2">
              <span>Total</span>
          ...
          <p className="text-[10px] text-gray-400 text-center border-t border-gray-100 pt-3">
            ...
            This invoice is generated by CAflow AI. Review before sending to client.
          </p>
        </div>
        ...
        <div className="px-6 pb-6 flex gap-2 print:hidden">
          ...
          <button
            onClick={handlePrint}
            className="flex items-center gap-1.5 px-4 border border-gray-200 text-gray-700 text-sm py-2 rounded-lg hover:bg-gray-50"
          >
...
export default function RetainerPage() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-gray-900">Monthly Retainer Tracker</h1>
        <p className="text-sm text-gray-500 mt-0.5">Track fixed-fee retainer clients, work done, and generate invoices</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-gray-500">Retainer Clients</span>
          </div>
          <p className="text-lg font-semibold text-gray-900">{loading ? "—" : retainerClients.length}</p>
          <p className="text-xs text-gray-400 mt-0.5">of {clients.length} total clients</p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          ...
          <p className="text-lg font-semibold text-gray-900">{loading ? "—" : formatPaise(totalRevenuePaise)}</p>
          <p className="text-xs text-gray-400 mt-0.5">Sum of all retainer fees</p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-gray-500">Invoiced This Month</span>
          </div>
          <p className="text-lg font-semibold text-gray-900">{invoicesThisMonth}</p>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          ...
          <p className="text-lg font-semibold text-gray-900">{outstandingCount}</p>
          <p className="text-xs text-gray-400 mt-0.5">Not yet invoiced</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-50 flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-gray-900">Retainer Clients</h2>
            <p className="text-xs text-gray-400 mt-0.5">Click &quot;Set Retainer&quot; to configure a client</p>
          </div>
        </div>

        {loading ? (
          <div className="px-5 py-8 text-center text-sm text-gray-400">Loading clients...</div>
        ) : clients.length === 0 ? (
          <div className="px-5 py-8 text-center text-sm text-gray-400">No clients found — add clients first</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-50">
                  <th className="text-left text-xs font-medium text-gray-400 px-5 py-3">Client Name</th>
                  <th className="text-right text-xs font-medium text-gray-400 px-3 py-3">Monthly Fee</th>
                  <th className="text-left text-xs font-medium text-gray-400 px-3 py-3">Services</th>
                  <th className="text-left text-xs font-medium text-gray-400 px-3 py-3">Work Status</th>
                  <th className="text-left text-xs font-medium text-gray-400 px-3 py-3">Invoice Status</th>
                    ...
                    <>
                      <tr key={client.id} className="hover:bg-gray-50/50">
                        <td className="px-5 py-3">
                          ...
                          {client.gstin && (
                            <p className="text-[10px] font-mono text-gray-400">{client.gstin}</p>
                          )}
                        </td>
                        <td className="px-3 py-3 text-right">
                          {config ? (
                            <span className="text-sm font-semibold text-gray-900">{formatPaise(config.feePaise)}</span>
                          ) : (
                          ...
                          {config && config.services.length > 0 ? (
                            <div className="flex flex-wrap gap-1">
                              ...
                              {config.services.length > 2 && (
                                <span className="text-[10px] text-gray-400">+{config.services.length - 2} more</span>
                              )}
                            </div>
                          ) : (
                        ...
                        <td className="px-3 py-3">
                          {config ? (
                            ...
                            >
                              <span className={`text-[10px] rounded-full px-1.5 py-0.5 font-medium ${completedCount === WORK_ITEMS.length ? "bg-green-100 text-green-700" : completedCount > 0 ? "bg-amber-100 text-amber-700" : "bg-gray-100 text-gray-500"}`}>
                      ...
                      {isExpanded && config && (
                        <tr key={`${client.id}-work`} className="bg-blue-50/30">
                          <td colSpan={6} className="px-5 py-3">
                            <p className="text-[11px] font-semibold text-gray-600 mb-2">Work Done This Month</p>
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        ...
        {showHistory && (
          invoices.length === 0 ? (
            <div className="px-5 py-6 text-sm text-gray-400 text-center">No invoices generated yet</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-50">
                    <th className="text-left text-xs font-medium text-gray-400 px-5 py-3">Invoice No</th>
                    <th className="text-left text-xs font-medium text-gray-400 px-3 py-3">Client</th>
                    <th className="text-left text-xs font-medium text-gray-400 px-3 py-3">Month</th>
                    <th className="text-right text-xs font-medium text-gray-400 px-3 py-3">Amount</th>
                    <th className="text-right text-xs font-medium text-gray-400 px-3 py-3">GST</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {invoices.map(inv => (
                    <tr key={inv.id} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-3 py-3 text-sm text-gray-900">{inv.clientName}</td>
                      <td className="px-3 py-3 text-xs text-gray-500">{inv.month}</td>
                      <td className="px-3 py-3 text-right text-sm text-gray-900">{formatPaise(inv.amountPaise)}</td>
                      <td className="px-3 py-3 text-right text-xs text-gray-500">{formatPaise(inv.gstPaise)}</td>
                      <td className="px-5 py-3 text-right text-sm font-semibold text-gray-900">{formatPaise(inv.totalPaise)}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="border-t border-gray-100 bg-gray-50/50">
                    <td colSpan={3} className="px-5 py-3 text-xs font-medium text-gray-500">Total</td>
                    ...
                    <td className="px-3 py-3 text-right text-xs font-semibold text-gray-500">
