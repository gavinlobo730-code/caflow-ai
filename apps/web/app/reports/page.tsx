  ...
  return (
    <div className="report-content space-y-6">
      <div className="border-b pb-4">
        <h2 className="text-lg font-semibold text-gray-900">GST Summary Report</h2>
        ...
        <table className="w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
          <tbody className="divide-y divide-gray-100">
            <tr className="bg-gray-50">
              <td className="px-4 py-2.5 text-gray-600">Taxable Value</td>
              <td className="px-4 py-2.5 text-right font-medium text-gray-900">
                {formatPaise(data.gstr1.taxable)}
              </td>
            </tr>
            <tr>
              <td className="px-4 py-2.5 text-gray-600">CGST</td>
              <td className="px-4 py-2.5 text-right font-medium text-gray-900">
                {formatPaise(data.gstr1.cgst)}
              </td>
            </tr>
            <tr className="bg-gray-50">
              <td className="px-4 py-2.5 text-gray-600">SGST</td>
              <td className="px-4 py-2.5 text-right font-medium text-gray-900">
                {formatPaise(data.gstr1.sgst)}
              </td>
            </tr>
            <tr>
              <td className="px-4 py-2.5 text-gray-600">IGST</td>
              <td className="px-4 py-2.5 text-right font-medium text-gray-900">
      ...
      {data.tds_deducted > 0 && (
        <section>
          <h3 className="text-sm font-semibold text-gray-700 mb-2 uppercase tracking-wide">TDS</h3>
          <div className="flex justify-between px-4 py-3 bg-gray-50 rounded-lg text-sm">
            <span className="text-gray-600">TDS Deducted (Purchase Invoices)</span>
            <span className="font-medium text-gray-900">{formatPaise(data.tds_deducted)}</span>
          </div>
        </section>
      )}
    </div>
...
function PLStatementReport({ data }: { data: PLData }) {
  ...
  return (
    <div className="report-content space-y-6">
      <div className="border-b pb-4">
        <h2 className="text-lg font-semibold text-gray-900">P&L Statement</h2>
        <p className="text-sm text-gray-500">
      ...
      {/* Revenue */}
      <section>
        <h3 className="text-sm font-semibold text-gray-700 mb-3 uppercase tracking-wide">Revenue</h3>
        <table className="w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
          <thead className="bg-gray-50 text-xs text-gray-500 uppercase">
            <tr>
          ...
          <tbody className="divide-y divide-gray-100">
            {data.revenue.length === 0 ? (
              ...
              data.revenue.map((r, i) => (
                <tr key={i}>
                  <td className="px-4 py-2.5 text-gray-600">{r.account}</td>
                  <td className="px-4 py-2.5 text-right text-gray-900">{formatPaise(r.total)}</td>
                </tr>
              ))
            )}
          </tbody>
...
function ComplianceStatusReport({ data }: { data: ComplianceRow[] }) {
  ...
  return (
    <div className="report-content space-y-4">
      <div className="border-b pb-4">
        <h2 className="text-lg font-semibold text-gray-900">Compliance Status Report</h2>
        <p className="text-sm text-gray-500">Current Financial Year — all clients</p>
      </div>
...
function OutstandingInvoicesReport({ data }: { data: OutstandingRow[] }) {
  ...
  return (
    <div className="report-content space-y-4">
      <div className="border-b pb-4">
        <h2 className="text-lg font-semibold text-gray-900">Outstanding Invoices Report</h2>
        <p className="text-sm text-gray-500">All draft/unpaid invoices — all clients</p>
      </div>
        ...
        <table className="w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
          ...
          <tbody className="divide-y divide-gray-100">
            {data.length === 0 ? (
              ...
              data.map((row) => (
                <tr key={row.id}>
                  <td className="px-4 py-2.5 text-gray-600">{row.clientName ?? row.clientId}</td>
                  <td className="px-4 py-2.5 text-gray-600">{formatDate(row.transactionDate)}</td>
                  <td className="px-4 py-2.5 font-medium text-gray-900">{row.partyName}</td>
                  <td className="px-4 py-2.5 text-right text-gray-900">{formatPaise(row.totalPaise)}</td>
                  <td className="px-4 py-2.5 text-right">
                    <span
...
function ReportViewer({ reportId, onClose }: ReportViewerProps) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      {/* Header */}
      <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 bg-gray-50 print:hidden">
        <Icon size={16} className="text-gray-500 shrink-0" />
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-gray-900">{reportDef.name}</p>
          <p className="text-xs text-gray-400">{reportDef.description}</p>
        </div>
        <button
          onClick={onClose}
          ...
          {(reportId === "gst_summary" || reportId === "pl_statement") && (
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                className="text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 min-w-[180px] bg-white"
              >
      ...
      <div className="p-6 max-w-5xl mx-auto space-y-6 report-print-root">
            ...
            {REPORTS.map((report) => {
              ...
              return (
                <button
                  ...
                  className="group flex items-start gap-4 p-5 bg-white rounded-xl border border-gray-100 hover:border-blue-200 hover:shadow-sm transition-all text-left"
                >
                  <div className="p-2.5 bg-gray-50 rounded-lg group-hover:bg-blue-50 transition-colors shrink-0">
        ...
        {activeReport && (
          <button
            ...
            className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-900 transition-colors print:hidden"
          >
            <ChevronUp size={14} />
            All Reports
          </button>
