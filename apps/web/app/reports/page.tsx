  ...
  return (
    <div className="report-content space-y-6">
      <div className="border-b pb-4">
        <h2 className="text-lg font-semibold text-ps-ink">GST Summary Report</h2>
      ...
      <section>
        <h3 className="text-sm font-semibold text-ps-body mb-3 uppercase tracking-wide">
          GSTR-1 — Outward Supplies
        </h3>
        <table className="w-full text-sm border border-ps-border rounded-lg overflow-hidden">
          <tbody className="divide-y divide-ps-muted">
            ...
            <tr className="bg-ps-bg">
              <td className="px-4 py-2.5 text-ps-label">SGST</td>
              <td className="px-4 py-2.5 text-right font-medium text-ps-ink">
                {formatPaise(data.gstr1.sgst)}
              </td>
            </tr>
            <tr>
              <td className="px-4 py-2.5 text-ps-label">IGST</td>
              <td className="px-4 py-2.5 text-right font-medium text-ps-ink">
      ...
      <section>
        ...
        <table className="w-full text-sm border border-ps-border rounded-lg overflow-hidden">
          <thead className="bg-ps-bg text-xs text-ps-label uppercase">
            <tr>
      ...
      {data.tds_deducted > 0 && (
        <section>
          <h3 className="text-sm font-semibold text-ps-body mb-2 uppercase tracking-wide">TDS</h3>
          <div className="flex justify-between px-4 py-3 bg-ps-bg rounded-lg text-sm">
            <span className="text-ps-label">TDS Deducted (Purchase Invoices)</span>
            <span className="font-medium text-ps-ink">{formatPaise(data.tds_deducted)}</span>
          </div>
        </section>
      )}
    </div>
...
function PLStatementReport({ data }: { data: PLData }) {
  ...
  return (
    <div className="report-content space-y-6">
      <div className="border-b pb-4">
        <h2 className="text-lg font-semibold text-ps-ink">P&L Statement</h2>
        <p className="text-sm text-ps-label">
      ...
      {/* Revenue */}
      <section>
        <h3 className="text-sm font-semibold text-ps-body mb-3 uppercase tracking-wide">Revenue</h3>
        <table className="w-full text-sm border border-ps-border rounded-lg overflow-hidden">
          <thead className="bg-ps-bg text-xs text-ps-label uppercase">
            <tr>
          ...
          <tbody className="divide-y divide-ps-muted">
            {data.revenue.length === 0 ? (
              ...
              data.revenue.map((r, i) => (
                <tr key={i}>
                  <td className="px-4 py-2.5 text-ps-label">{r.account}</td>
                  <td className="px-4 py-2.5 text-right text-ps-ink">{formatPaise(r.total)}</td>
                </tr>
              ))
            )}
          </tbody>
      ...
      {/* Expenses */}
      <section>
        <h3 className="text-sm font-semibold text-ps-body mb-3 uppercase tracking-wide">Expenses</h3>
        <table className="w-full text-sm border border-ps-border rounded-lg overflow-hidden">
          <thead className="bg-ps-bg text-xs text-ps-label uppercase">
            <tr>
              <th className="px-4 py-2 text-left">Account / Party</th>
...
function ComplianceStatusReport({ data }: { data: ComplianceRow[] }) {
  ...
  return (
    <div className="report-content space-y-4">
      <div className="border-b pb-4">
        <h2 className="text-lg font-semibold text-ps-ink">Compliance Status Report</h2>
        <p className="text-sm text-ps-label">Current Financial Year — all clients</p>
      </div>

      {/* Legend */}
      <div className="flex items-center gap-4 text-xs text-ps-label">
        ...
        <table className="w-full text-sm border border-ps-border rounded-lg overflow-hidden">
          ...
          <tbody className="divide-y divide-ps-muted">
            {data.length === 0 ? (
              ...
              data.map((row) => (
                <tr key={row.clientId}>
                  <td className="px-4 py-3 font-medium text-ps-ink">{row.clientName}</td>
...
function OutstandingInvoicesReport({ data }: { data: OutstandingRow[] }) {
  ...
  return (
    <div className="report-content space-y-4">
      <div className="border-b pb-4">
        <h2 className="text-lg font-semibold text-ps-ink">Outstanding Invoices Report</h2>
        <p className="text-sm text-ps-label">All draft/unpaid invoices — all clients</p>
      </div>
        ...
        <table className="w-full text-sm border border-ps-border rounded-lg overflow-hidden">
          ...
          <tbody className="divide-y divide-ps-muted">
            {data.length === 0 ? (
              ...
              data.map((row) => (
                <tr key={row.id}>
                  <td className="px-4 py-2.5 text-ps-label">{row.clientName ?? row.clientId}</td>
                  <td className="px-4 py-2.5 text-ps-label">{formatDate(row.transactionDate)}</td>
                  <td className="px-4 py-2.5 font-medium text-ps-ink">{row.partyName}</td>
                  <td className="px-4 py-2.5 text-right text-ps-ink">{formatPaise(row.totalPaise)}</td>
                  <td className="px-4 py-2.5 text-right">
                    <span
    ...
    <div className="bg-white rounded-xl border border-ps-border shadow-sm overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b border-ps-muted bg-ps-bg print:hidden">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-ps-hint">{reportDef.description}</p>
        </div>
        <button
          onClick={onClose}
          className="p-1.5 rounded-lg text-ps-hint hover:text-ps-label hover:bg-ps-muted transition-colors"
          aria-label="Close"
        >
          <X size={14} />
        </button>
        ...
        <div className="flex flex-wrap gap-3 items-end">
          ...
          {(reportId === "gst_summary" || reportId === "pl_statement") && (
            <div className="flex flex-col gap-1">
              <label className="text-xs text-ps-label font-medium">Client</label>
          ...
          {reportId === "gst_summary" && (
            <div className="flex flex-col gap-1">
              <label className="text-xs text-ps-label font-medium">Month</label>
              <input
                type="month"
                value={selectedMonth}
          ...
          {data && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 border border-ps-border text-ps-label text-sm font-medium rounded-lg hover:bg-ps-bg transition-colors"
            >
              <Printer size={14} />
              Print / Export
            </button>
      ...
      <div className="p-5">
        ...
        {loading && (
          <div className="flex items-center justify-center py-12 gap-3 text-ps-hint">
      ...
      <div className="p-6 max-w-5xl mx-auto space-y-6 report-print-root">
        ...
        {!activeReport && (
          <div className="print:hidden">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-ps-ink group-hover:text-blue-700 transition-colors">
                  Cash Flow Forecast
                </p>
                <p className="text-xs text-ps-hint mt-0.5 leading-relaxed">
                  6-month projection based on unpaid invoices, loan EMIs, and GST dues
                </p>
              </div>
              <ChevronDown
                size={14}
                className="text-ps-disabled group-hover:text-blue-600 transition-colors mt-1 shrink-0 -rotate-90"
              />
            </Link>
          </div>
        )}
            ...
            {REPORTS.map((report) => {
              ...
              return (
                ...
                >
                  ...
                  <ChevronDown
                    size={14}
                    className="text-ps-disabled group-hover:text-blue-600 transition-colors mt-1 shrink-0"
                  />
                </button>
              );
            })}
        ...
        {activeReport && (
          <button
            ...
            className="flex items-center gap-1.5 text-sm text-ps-label hover:text-ps-ink transition-colors print:hidden"
          >
            <ChevronUp size={14} />
            All Reports
          </button>
