    ...
    return (
      <div className="px-6 py-4">
        <div className="space-y-3">
          {[...Array(4)].map((_, i) => <div key={i} className="h-10 rounded-lg bg-gray-50 animate-pulse" />)}
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-4 max-w-5xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <p className="text-[10px] text-gray-500 mb-1">Outstanding Payable</p>
          <p className="text-lg font-bold text-orange-700 tabular-nums">{fmt(totalPayable)}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <p className="text-[10px] text-gray-500 mb-1">Total Bills (FY)</p>
          <p className="text-lg font-bold text-gray-800 tabular-nums">{fmt(totalThisFy)}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <p className="text-[10px] text-gray-500 mb-1">Bills This FY</p>
          <p className="text-lg font-bold text-gray-800 tabular-nums">{bills.length}</p>
        </div>
      </div>

      <div className="flex justify-between items-center">
        <p className="text-xs font-semibold text-gray-700">Purchase Bills — FY {financialYear}</p>
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-100 p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-gray-900">New Purchase Bill</h3>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="col-span-2">
              ...
              <select value={vendorId} onChange={(e) => setVendorId(e.target.value)} className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                <option value="">— Select vendor —</option>
                {vendors.map((v) => <option key={v.id} value={v.id}>{v.name}{v.tds_applicable ? ` (TDS ${(v.tds_rate_bps / 100).toFixed(0)}%)` : ""}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Vendor Invoice No.</label>
              <input value={billNo} onChange={(e) => setBillNo(e.target.value)} placeholder="INV-001" className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Bill Date *</label>
              <input type="date" value={billDate} onChange={(e) => setBillDate(e.target.value)} className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Due Date</label>
              <input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
            ...
            <table className="w-full text-xs">
                  ...
                  return (
                    <tr key={idx}>
                      <td className="py-1.5 pr-2">
                        <input value={line.description} onChange={(e) => setLine(idx, { description: e.target.value })} placeholder="Item description" className="w-full px-2 py-1 border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 text-xs" />
                      </td>
                      <td className="py-1.5 px-1">
                        <input value={line.hsn_sac} onChange={(e) => setLine(idx, { hsn_sac: e.target.value })} placeholder="HSN/SAC" className="w-full px-2 py-1 border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 text-xs" />
                      </td>
                      <td className="py-1.5 px-1">
                        <select value={line.expense_account_id} onChange={(e) => setLine(idx, { expense_account_id: e.target.value })} className="w-full px-1 py-1 border border-gray-200 rounded focus:outline-none text-xs">
                          <option value="">— Account —</option>
                          {accounts.map((a) => <option key={a.id} value={a.id}>{a.account_code} {a.account_name}</option>)}
                        </select>
                      </td>
                      <td className="py-1.5 px-1">
                        <input type="number" min="0.001" step="0.001" value={line.quantity} onChange={(e) => setLine(idx, { quantity: parseFloat(e.target.value) || 1 })} className="w-full px-2 py-1 border border-gray-200 rounded focus:outline-none text-xs text-right" />
                      </td>
                      <td className="py-1.5 px-1">
                        <input type="number" min="0" step="0.01" value={line.rate || ""} onChange={(e) => setLine(idx, { rate: parseFloat(e.target.value) || 0 })} placeholder="0.00" className="w-full px-2 py-1 border border-gray-200 rounded focus:outline-none text-xs text-right" />
                      </td>
                      <td className="py-1.5 px-1">
                        <select value={line.gst_rate_bps} onChange={(e) => setLine(idx, { gst_rate_bps: parseInt(e.target.value) })} className="w-full px-1 py-1 border border-gray-200 rounded focus:outline-none text-xs">
                          {GST_RATES.map((r) => <option key={r.bps} value={r.bps}>{r.label}</option>)}
                        </select>
                      </td>
                      <td className="py-1.5 px-1 text-right font-mono text-gray-700">
                        {g.taxable_paise > 0 ? fmt(g.line_total) : "—"}
                      </td>
                      <td className="py-1.5 pl-1">
                        {lines.length > 1 && <button onClick={() => setLines((prev) => prev.filter((_, i) => i !== idx))} className="text-gray-300 hover:text-red-400 font-bold">×</button>}
                      </td>
                    </tr>
                  );
                })}
              ...
              <tfoot>
                <tr className="border-t border-gray-100 text-xs font-semibold">
                  <td colSpan={6} className="pt-2 text-right text-gray-500 pr-2">Taxable</td>
                  <td className="pt-2 text-right font-mono text-gray-700 px-1">{fmt(totals.taxable)}</td>
                  <td />
                </tr>
                <tr className="text-xs">
                  <td colSpan={6} className="text-right text-gray-400 pr-2">Total GST</td>
                  <td className="text-right font-mono text-gray-500 px-1">{fmt(totals.gst)}</td>
                  <td />
                </tr>
                {tds_paise > 0 && (
                  <tr className="text-xs">
                ...
                <tr className="text-xs font-bold border-t border-gray-200">
                  <td colSpan={6} className="pt-1 text-right text-gray-800 pr-2">Net Payable</td>
                  <td className="pt-1 text-right font-mono text-gray-900 px-1">{fmt(net_payable)}</td>
                  <td />
                </tr>
              </tfoot>
            </table>
          ...
          <div className="flex gap-3 justify-end pt-1">
            <button onClick={() => setShowForm(false)} className="text-xs px-4 py-2 border border-gray-200 rounded-lg hover:bg-gray-50">Cancel</button>
      ...
      {/* Bills table */}
      {loading ? (
        <div className="space-y-2">{[...Array(4)].map((_, i) => <div key={i} className="h-10 rounded-lg bg-gray-50 animate-pulse" />)}</div>
      ) : bills.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-100 text-center py-16">
          <p className="text-sm text-gray-400">No purchase bills for FY {financialYear}.</p>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-gray-50">
                {bills.map((b) => (
                  <tr key={b.id} className="hover:bg-gray-50">
                    <td className="px-4 py-2.5 font-mono text-gray-600 text-[10px]">{b.our_reference ?? "—"}</td>
                    <td className="px-3 py-2.5 text-gray-600">{b.bill_no ?? "—"}{b.is_ai_extracted && <span className="ml-1 text-[9px] bg-amber-100 text-amber-600 px-1 rounded">AI</span>}</td>
                    <td className="px-3 py-2.5 font-medium text-gray-800">{(b.vendors as { name: string } | undefined)?.name ?? "—"}</td>
                    <td className="px-3 py-2.5 text-gray-500">{b.bill_date}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-gray-700">{fmt(b.taxable_amount_paise)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-gray-500">{fmt(b.total_gst_paise)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-blue-600">{b.tds_paise > 0 ? fmt(b.tds_paise) : "—"}</td>
                    <td className="px-3 py-2.5 text-right font-mono font-semibold text-gray-800">{fmt(b.net_payable_paise)}</td>
                    <td className="px-3 py-2.5">
                      <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-medium ${STATUS_COLORS[b.status] ?? "bg-gray-100 text-gray-600"}`}>{b.status}</span>
                    </td>
...
function Vendors({ clientId }: { clientId: string; financialYear: string }) {
  ...
  return (
    <div className="space-y-4 max-w-4xl">
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold text-gray-700">{vendors.length} vendor{vendors.length !== 1 ? "s" : ""}</p>
        <div className="flex gap-2">
          <button onClick={load} className="p-1.5 rounded border border-gray-200 hover:bg-gray-50"><RefreshCw size={13} className={loading ? "animate-spin text-gray-400" : "text-gray-400"} /></button>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              ...
              <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Vendor legal name" className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">GSTIN</label>
              <input value={gstin} onChange={(e) => setGstin(e.target.value.toUpperCase())} placeholder="27AABCS1429B1Z5" maxLength={15} className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">PAN</label>
              <input value={pan} onChange={(e) => setPan(e.target.value.toUpperCase())} placeholder="ABCDE1234F" maxLength={10} className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Email</label>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Phone</label>
              <input value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Opening Balance (₹ payable)</label>
              <input type="number" min="0" step="0.01" value={openingBalance} onChange={(e) => setOpeningBalance(e.target.value)} placeholder="0.00" className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
            ...
            {tdsApplicable && (
              <div className="grid grid-cols-2 gap-3 pt-1">
                <div>
                  ...
                  <select value={tdsSection} onChange={(e) => { setTdsSection(e.target.value); setTdsRate(String(TDS_DEFAULT_RATES[e.target.value] / 100)); }} className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div>
                  ...
                  <input type="number" min="0" max="30" step="0.1" value={tdsRate} onChange={(e) => setTdsRate(e.target.value)} className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
              </div>
            )}
          </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
          <table className="w-full text-xs">
            ...
            <tbody className="divide-y divide-gray-50">
              {vendors.length === 0 ? (
                <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400">No vendors added yet.</td></tr>
              ) : vendors.map((v) => (
                <tr key={v.id} className="hover:bg-gray-50">
                  <td className="px-4 py-2.5 font-medium text-gray-800">{v.name}</td>
                  <td className="px-3 py-2.5 font-mono text-gray-500 text-[10px]">{v.gstin ?? "—"}</td>
                  ...
                  <td className="px-3 py-2.5 text-gray-500">{v.tds_section ?? "—"}</td>
                  <td className="px-3 py-2.5 text-right text-gray-600">{v.tds_rate_bps > 0 ? `${(v.tds_rate_bps / 100).toFixed(1)}%` : "—"}</td>
                  <td className="px-4 py-2.5 text-right font-mono text-gray-700">{v.opening_balance_paise > 0 ? fmt(v.opening_balance_paise) : "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function Payments({ clientId, financialYear }: { clientId: string; financialYear: string }) {
  ...
  return (
    <div className="space-y-4 max-w-4xl">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <p className="text-[10px] text-gray-500 mb-1">Total Paid (FY)</p>
          <p className="text-lg font-bold text-green-700 tabular-nums">{fmt(totalPaid)}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <p className="text-[10px] text-gray-500 mb-1">Transactions</p>
          <p className="text-lg font-bold text-gray-800 tabular-nums">{payments.length}</p>
        </div>
      </div>

      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold text-gray-700">Payments — FY {financialYear}</p>
        <div className="flex gap-2">
          <button onClick={load} className="p-1.5 rounded border border-gray-200 hover:bg-gray-50"><RefreshCw size={13} className={loading ? "animate-spin text-gray-400" : "text-gray-400"} /></button>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
            <div className="col-span-2 lg:col-span-1">
              <label className="block text-xs font-medium text-gray-600 mb-1">Vendor *</label>
              <select value={vendorId} onChange={(e) => { setVendorId(e.target.value); if (e.target.value) loadOpenBills(e.target.value); }} className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                <option value="">— Select vendor —</option>
                {vendors.map((v) => <option key={v.id} value={v.id}>{v.name}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Against Bill (optional)</label>
              <select value={billId} onChange={(e) => setBillId(e.target.value)} className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div>
              ...
              <input type="date" value={payDate} onChange={(e) => setPayDate(e.target.value)} className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Amount (₹) *</label>
              <input type="number" min="0" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Mode</label>
              <select value={mode} onChange={(e) => setMode(e.target.value)} className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div>
              ...
              <input value={refNo} onChange={(e) => setRefNo(e.target.value)} placeholder="UTR / cheque no." className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
          <div className="flex gap-3 justify-end">
            <button onClick={() => setShowForm(false)} className="text-xs px-4 py-2 border border-gray-200 rounded-lg hover:bg-gray-50">Cancel</button>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
          <table className="w-full text-xs">
            ...
            <tbody className="divide-y divide-gray-50">
              {payments.length === 0 ? (
                <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400">No payments for FY {financialYear}.</td></tr>
              ) : payments.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50">
                  <td className="px-4 py-2.5 font-mono text-gray-600 text-[10px]">{p.payment_no}</td>
                  <td className="px-3 py-2.5 text-gray-500">{p.payment_date}</td>
                  <td className="px-3 py-2.5 font-medium text-gray-800">{(p.vendors as { name: string } | undefined)?.name ?? "—"}</td>
                  <td className="px-3 py-2.5 text-right font-mono font-semibold text-gray-800">{fmt(p.amount_paise)}</td>
                  <td className="px-3 py-2.5 text-gray-500 capitalize">{p.payment_mode}</td>
                  <td className="px-4 py-2.5 text-gray-400 text-[10px]">{p.reference_no ?? "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
