...
function gradeBadge(g: Grade) {
  ...
  return map[g] ?? "bg-gray-700 text-gray-300";
}
    ...
    return (
      <div className="p-6 space-y-4 animate-pulse">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[1, 2, 3, 4, 5, 6, 7].map((i) => <div key={i} className="h-20 bg-white/[0.05] rounded-xl" />)}
        </div>
      </div>
    );
  }
      ...
      <div className="p-6 space-y-4">
        ...
        <Card className="bg-gray-800 border-gray-700">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-sm text-slate-400 mb-4">No health score calculated yet</p>
            <button
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-base font-semibold text-white">Client Health</h1>
        <button
          onClick={handleRecalculate}
          disabled={recalculating}
          className="flex items-center gap-1.5 text-xs text-slate-400 border border-gray-700 px-3 py-1.5 rounded hover:bg-gray-800 disabled:opacity-50"
        >
      ...
      {/* Dimensions */}
      <div>
        <h2 className="text-sm font-semibold text-white mb-3">Dimension Scores</h2>
              ...
              <Card key={key} className="bg-gray-800 border-gray-700">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-xs text-slate-400">{DIMENSION_LABELS[key]}</p>
              ...
              <Card key={a.id} className="bg-gray-800 border-gray-700">
                <CardContent className="p-4">
                  <div className="flex items-center gap-2 mb-1">
                    <Badge className={`text-[10px] ${SEVERITY_COLORS[a.severity] ?? "bg-gray-700 text-gray-300"}`}>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold text-white">Active Overrides ({overrides.length})</h2>
          <button
        ...
        <Card className="bg-gray-800 border-gray-700">
          <CardContent className="p-0">
            {overrides.length === 0 ? (
              <div className="py-8 text-center"><p className="text-sm text-slate-500">No active overrides</p></div>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-xs text-slate-500 border-b border-gray-700">
                  ...
                  {overrides.map((o) => (
                    <tr key={o.id} className="hover:bg-gray-700/30">
                      <td className="px-5 py-3">
                        <Badge className="bg-emerald-800 text-emerald-300 text-[10px]">
                      ...
                      <td className="px-3 py-3 text-slate-300 text-xs max-w-xs truncate">{o.reason}</td>
                      <td className="px-3 py-3 text-slate-400 text-xs">{formatDate(o.expires_at)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="fixed inset-0 bg-[#0F172A]/75 flex items-center justify-center z-50 px-4">
          <div className="bg-gray-900 border border-gray-700 rounded-xl shadow-xl p-6 w-full max-w-md">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-sm font-semibold text-white">Add Override</h2>
              <button onClick={() => setOverrideModal(false)} className="text-slate-500 hover:text-white"><X size={16} /></button>
            </div>
            <div className="space-y-3">
              ...
              <div>
                <label className="text-xs text-slate-400">Override Score (0–100) *</label>
                <input
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full mt-1 px-3 py-2 text-sm bg-gray-800 border border-gray-600 rounded-md text-white resize-none focus:outline-none focus:ring-2 focus:ring-emerald-500"
            ...
            <div className="flex gap-2 mt-5">
              <button onClick={() => setOverrideModal(false)} className="flex-1 text-sm text-slate-400 border border-gray-700 py-2 rounded-md hover:bg-gray-800">Cancel</button>
              <button
