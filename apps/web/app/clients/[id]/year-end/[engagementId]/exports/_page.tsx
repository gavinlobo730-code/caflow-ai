...
export default function ExportsPage() {
  ...
  return (
    <div className="p-6 space-y-5 max-w-4xl mx-auto">
          ...
          return (
            <div
              ...
              className={`bg-white rounded-xl border p-4 space-y-3 flex flex-col ${
                cfg.primary ? "border-blue-200 bg-blue-50/30" : "border-ps-muted"
              }`}
            >
              <div className="flex items-start gap-2">
                <Icon size={16} className={cfg.primary ? "text-blue-600" : "text-ps-label"} />
              ...
              <div className="flex gap-2 mt-auto">
                ...
                {latest && (
                  <button
                    ...
                    className="flex items-center gap-1 text-xs px-2 py-2 border border-ps-border rounded-lg hover:bg-ps-bg text-ps-label disabled:opacity-50"
      ...
      {exports.length > 0 && (
        <div className="bg-white rounded-xl border border-ps-muted overflow-hidden">
          <div className="px-4 py-3 border-b border-ps-bg">
            <p className="text-xs font-semibold text-ps-body">Export History</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-ps-bg">
                {exports.map((exp) => (
                  <tr key={exp.id} className="hover:bg-ps-bg">
                    <td className="px-4 py-2.5 text-ps-body">{TYPE_LABELS[exp.export_type] ?? exp.export_type}</td>
                    ...
                    <td className="px-3 py-2.5">
                      {(exp.is_draft ?? false) ? (
                        <span className="text-3xs font-medium px-1.5 py-0.5 rounded-full bg-state-attention-surface text-state-attention">Draft</span>
                      ) : (
      ...
      {exports.length === 0 && (
        <div className="bg-white rounded-xl border border-ps-muted text-center py-10">
          <p className="text-xs text-ps-hint">No exports generated yet. Click Generate above.</p>
        </div>
      )}
    </div>
  );
