  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      ...
      <div className="rounded border p-4 bg-amber-50">
        <p className="text-xs text-gray-500">TDS Returns</p>
    ...
    <div className="space-y-4">
      ...
      {loading ? <p className="text-sm text-gray-500">Loading…</p> : (
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={(r.id as string) ?? i} className="border-b hover:bg-gray-50">
            ...
            {rows.length === 0 && (
              <tr><td colSpan={6} className="px-3 py-4 text-center text-gray-400">No deductions recorded.</td></tr>
            )}
          </tbody>
        </table>
      )}
      ...
      {loading ? <p className="text-sm text-gray-500">Loading…</p> : (
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            ...
            {rows.length === 0 && (
              <tr><td colSpan={7} className="px-3 py-4 text-center text-gray-400">No challans yet.</td></tr>
            )}
          </tbody>
        </table>
      )}
      ...
      {loading ? <p className="text-sm text-gray-500">Loading…</p> : (
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
                ...
                <td className="px-3 py-2 space-x-2">
                  {r.status === "pending" && (
                    <button onClick={() => updateStatus(r.id as string, "prepared")}
                      className="text-xs px-2 py-0.5 border rounded hover:bg-gray-100">Prepare</button>
                  )}
                  {r.status === "prepared" && (
            ...
            {rows.length === 0 && (
              <tr><td colSpan={6} className="px-3 py-4 text-center text-gray-400">No TDS returns yet.</td></tr>
            )}
          </tbody>
        </table>
      )}
            ...
            return (
              <div className="space-y-2 text-sm">
                ...
                {(mismatched?.length ?? 0) > 0 && mismatched.map((m, i) => (
                  <div key={i} className="text-xs text-gray-700 border rounded p-2">
      ...
      {loading ? <p className="text-sm text-gray-500">Loading…</p> : (
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            ...
            {rows.length === 0 && (
              <tr><td colSpan={6} className="px-3 py-4 text-center text-gray-400">No certificates generated.</td></tr>
            )}
          </tbody>
        </table>
      )}
...
export default function TDSWorkspacePage() {
  ...
  if (!clientId || clientId === "_placeholder") {
    return <p className="text-sm text-gray-500 p-6">Select a client to view TDS workspace.</p>;
  }
