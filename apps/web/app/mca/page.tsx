    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-[#131620] rounded-xl shadow-xl w-full max-w-md p-6 space-y-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-white/85">Add MCA Filing</h3>
        ...
        <div className="space-y-3">
          <div>
            ...
            <select className="w-full border border-white/[0.07] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={clientId} onChange={e => setClientId(e.target.value)}>
              <option value="">Select client…</option>
              {clients.map(c => <option key={c.id} value={c.id}>{c.client_name}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs font-medium text-white/65 block mb-1">CIN / LLPIN</label>
            <input className="w-full border border-white/[0.07] rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="U17200MH2010PTC205678" value={cin} onChange={e => setCin(e.target.value.toUpperCase())} />
          </div>
          <div>
            <label className="text-xs font-medium text-white/65 block mb-1">Form Type</label>
            <select className="w-full border border-white/[0.07] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={formType} onChange={e => setFormType(e.target.value)}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input className="w-full border border-white/[0.07] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={period} onChange={e => setPeriod(e.target.value)} placeholder="FY 2025-26" />
            </div>
            <div>
              <label className="text-xs font-medium text-white/65 block mb-1">Due Date</label>
              <input type="date" className="w-full border border-white/[0.07] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={dueDate} onChange={e => setDueDate(e.target.value)} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-medium text-white/65 block mb-1">Status</label>
              <select className="w-full border border-white/[0.07] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={status} onChange={e => setStatus(e.target.value as FilingStatus)}>
            ...
            <div>
              <label className="text-xs font-medium text-white/65 block mb-1">SRN (if filed)</label>
              <input className="w-full border border-white/[0.07] rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500" value={srn} onChange={e => setSrn(e.target.value)} placeholder="SRN12345678" />
            </div>
          </div>
          <div>
            <label className="text-xs font-medium text-white/65 block mb-1">Notes</label>
            <input className="w-full border border-white/[0.07] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={notes} onChange={e => setNotes(e.target.value)} placeholder="Optional" />
          </div>
        </div>
...
export default function MCAPage() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-white/85">MCA / ROC Module</h1>
          <p className="text-sm text-white/40 mt-0.5">Ministry of Corporate Affairs — Companies Act 2013 compliance tracker</p>
        </div>
        {activeTab === 1 && (
      ...
      {activeTab === 0 && (
        <div className="bg-[#131620] rounded-xl border border-white/[0.05] overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-50">
            <h2 className="text-sm font-semibold text-white/85">Company Clients</h2>
            <p className="text-xs text-white/30 mt-0.5">Companies Act 2013 — incorporated entities</p>
          </div>
                  ...
                  <tr key={c.id} className="hover:bg-[#0e1017]/50">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-white/85">{c.client_name}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-xs font-mono text-white/55">{c.cin}</td>
                    <td className="px-4 py-3 text-xs text-white/55">{fmtDate(c.incorp_date)}</td>
                    <td className="px-4 py-3 text-sm text-white/85">{fmtLakhs(c.auth_capital_paise)}</td>
                    <td className="px-4 py-3 text-sm text-white/85">{fmtLakhs(c.paidup_capital_paise)}</td>
                    <td className="px-4 py-3 text-xs text-white/55">{c.regd_office}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {activeTab === 1 && (
        <div className="bg-[#131620] rounded-xl border border-white/[0.05] overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-50">
            <h2 className="text-sm font-semibold text-white/85">ROC Filing Tracker</h2>
            <p className="text-xs text-white/30 mt-0.5">Companies Act 2013 — ROC/MCA filing obligations</p>
          </div>
                ...
                <tbody className="divide-y divide-white/[0.03]">
                  {filings.map(f => (
                    <tr key={f.id} className="hover:bg-[#0e1017]/50">
                      <td className="px-4 py-3 text-sm font-medium text-white/85">{f.client_name}</td>
                      <td className="px-4 py-3 text-xs font-mono text-white/55">{f.company_cin}</td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-xs text-white/55">{f.period}</td>
                      <td className="px-4 py-3 text-xs text-white/55">{fmtDate(f.due_date)}</td>
                      <td className="px-4 py-3 text-xs text-white/55">{f.filed_date ? fmtDate(f.filed_date) : "—"}</td>
                      <td className="px-4 py-3 text-xs font-mono text-white/55">{f.srn || "—"}</td>
                      <td className="px-4 py-3">
      ...
      {activeTab === 2 && (
        <div className="bg-[#131620] rounded-xl border border-white/[0.05] overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-50">
            <h2 className="text-sm font-semibold text-white/85">Director Master</h2>
            <p className="text-xs text-white/30 mt-0.5">Director KYC due date: 30 Sep annually — Rule 12A Companies (Appointment and Qualification of Directors) Rules</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                ...
                {directors.map(d => {
                  ...
                  return (
                    <tr key={d.id} className="hover:bg-[#0e1017]/50">
                      <td className="px-4 py-3 text-sm font-medium text-white/85">{d.name}</td>
                      <td className="px-4 py-3 text-xs font-mono text-white/55">{d.din}</td>
                      <td className="px-4 py-3 text-xs font-mono text-white/55">{d.pan}</td>
                      <td className="px-4 py-3 text-xs text-white/55">{d.designation}</td>
                      <td className="px-4 py-3 text-xs text-white/55">{fmtDate(d.appointment_date)}</td>
                      <td className="px-4 py-3 text-xs text-white/55">{fmtDate(d.kyc_due_date)}</td>
                      <td className="px-4 py-3">
      ...
      {activeTab === 3 && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {KEY_DEADLINES.map(d => (
              <div key={d.label} className="bg-[#131620] rounded-xl border border-white/[0.05] p-4">
                ...
                <p className="text-xs text-white/30 mt-1">{d.note}</p>
                <p className={`text-xs font-medium mt-2 ${d.daysLeft < 30 ? "text-amber-600" : "text-white/40"}`}>
